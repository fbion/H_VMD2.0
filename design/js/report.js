...
!function (t) {
    ...
    t.fn.colorPicker = function (e) {
        ...
        this.each(function () {
            ...
            }), e && e.onColorChange ? o.data("onColorChange", e.onColorChange) : o.data("onColorChange", function () {
            }), r++
        });
    ...
    if (type == 'fp') {
        ...
        if (this.times) {
            ...
            vmd(me.el.dom).find('.htBorders .wtBorder.area').eq(4).removeClass('corner').css({
                ...
                'cursor': 'se-resize'
            }).unbind('mousedown').on('mousedown', resizeRange)
        }
    }
    if (type == 'nested') {
        var resizeRange = function () {
        ...
        if (this.nestedTimes) {
            ...
            vmd(sheetHot_me.el.dom).find('.htBorders .wtBorder.current').eq(4).removeClass('corner').css(cssText).unbind('mousedown').on('mousedown', resizeRange)

            vmd(sheetHot_me.el.dom).find('.htBorders .wtBorder.area').eq(4).removeClass('corner').css(cssText).unbind('mousedown').on('mousedown', resizeRange)
        }
    }
    if (type == null) {
        this.cornerSet = false;
...
Handsontable.Core.prototype.contextMenu_removeRow = function () {
    ...
    this.handleNestedTableRemoveRow(c, count)
    this.nestedTableArrayTemp = this.nestedTableArray;
    //this.alter('remove_row', c, count)
...
Handsontable.Core.prototype.insertRow = function (val, position) {
    ...
    // 选择行为标题行时不允许再插入行，给出提示信息 2020.3.25
    if (this.getCellMeta(c, 0).rowtype == 'title') {
        vmd.tip("标题行不允许插入行，请在其他行进行插入行操作！", "error");
        return;
    }
    //插入后合并单元格处理
    var mObj = this.getPlugin('MergeCells').mergedCellsCollection.mergedCells;
    ...
    //更新填报规则
    this.updateSubmitArray('addRow', c, val)
...
Handsontable.Core.prototype.insertCol = function (val) {
    ...
    //更新填报规则
    this.updateSubmitArray('addCol', c, val)
}
/**
 *  当表格增加删除行列时更新填报规则
 */
Handsontable.Core.prototype.updateSubmitArray = function (type, col, val) {
    if (typeof sheetHot != 'undefined') {
        var submitarray = sheetHot.submitArray;
        for (var i = 0; i < submitarray.length; i++) {
            for (var j = 0; j < submitarray[i].banding.length; j++) {
                if (submitarray[i].banding[j].cell != '') {
                    var cell = submitarray[i].banding[j].cell;
                    var colStr = cell.match(/[a-zA-Z]*/)[0];
                    var rowStr = cell.match(/[a-zA-Z]*(\S*)/)[1];
                    var colNum = this.engToNum(colStr) - 1;
                    var rowNum = parseInt(rowStr);
                    //判断删除行或列是否影响绑定的单元格
                    if (colNum >= col) {

                        if (type == 'addCol') {
                            sheetHot.submitArray[i].banding[j].cell = this.numToEng(++colNum) + rowStr;
                        } else if (type == 'removeCol') {
                            if (colNum < 1) {
                                sheetHot.submitArray[i].banding[j].cell = '';
                                break;
                            }
                            sheetHot.submitArray[i].banding[j].cell = this.numToEng(--colNum) + rowStr;
                        }
                    ...
                    if (rowNum > col) {
                        if (type == 'addRow') {
                            sheetHot.submitArray[i].banding[j].cell = colStr + (++rowNum);
    ...
    if (xds.eastlayout && xds.eastlayout.reportInst){
        var cell = this.dealInvert()[0];
        var info = this.getCellMeta(cell.sr, cell.sc);
        var cP = xds.eastlayout.reportInst;
        cP.setPropertyInfo(info, this, cell);
    }
        ...
        for (var c = 0; c < cellCount; c++) {
            ...
            if (gcell) {
                ...
                if (style && style.numbers && style.numbers[m]) {
                    ...
                    } else if (numinfo.type == "7") {
                        attribute.numberInfo.allSortCom.value = "myText";
                    } else if (numinfo.type == "8") {
                        ...
                        for (var c = arrys.sc; c <= arrys.ec; c++) {
                            ...
                            switch (flag) {
                                ...
                                case 9:
                                    ...
                                    break;
                                case 'saveRlue':
                                    parent.xds.eastlayout.reportInst.hwReporting.enter_store_express.setValue(exp)
                                    parent.sheetHot.addSumitBlue(exp);
                                    break;
                                case 'menuID':
...
Handsontable.Core.prototype.submitLi = function (ul, info, canclick) {
    ...
    if (flag) {
        ...
        li.onclick = function () {
            ...
            var blue = checkInst.enter_store_express;
            var temp = li.parentElement.childNodes;
            for (var n = 0; n < temp.length; n++) {
                temp[n].style.backgroundColor = 'white'
                temp[n].style.color = "#000"
            ...
            for (var i = 0; i < that.submitArray.length; i++) {
                if (li.id == that.submitArray[i].id) {
                    ...
                    blue.setValue(that.submitArray[i].saverule)
                }
            }
...
Handsontable.Core.prototype.addSumitBlue = function (info) {
    var that = this;
    for (var i = 0; i < that.submitArray.length; i++) {
        if (that.submitId == that.submitArray[i].id) {
            that.submitArray[i].saverule = info;
        }
    }
    console.log(that.submitArray)
...
Handsontable.Core.prototype.saveSubmitRules = function (report) {
    ...
    for (var i = 0; i < this.submitArray.length; i++) {
        ...
        report.main.submitrules.push({
            ...
            saverule: this.submitArray[i].saverule
        })
    }
}
        ...
        if (eve && eve.change) {
            if (typeof eve.change == 'string') {
                ...
                for (var i = 0; i < event.length; i++) {
                    ...
                    } else if (event[i].name && event[i].javascript) {
                        text = "/*" + event[i].javascript + "*/"
                    } else if (event[i].name && event[i].rptName) {
...
Ext.define('vmd.report.ClipBordParse', {
    ...
    parseFontFamily: function (e, t, n) {
        ...
        if (t) {
            ...
            if (!r) return;
            var i = r.indexOf(",");
            -1 != i && (r = r.slice(0, i)), '"' == r[0] && (r = r.slice(1, r.length - 1)), r = r.toLowerCase();
