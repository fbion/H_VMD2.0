Ext.define('vmd.d.webchart.util.DataParse',{},function(){
	
Ext.override(hwchart.util.DataParse,{
    ...
    seriesMapping:[
       ...
       {	"seriseType": "faultLine",
            "fields":["id","", "type","wallid","walltype","x","y"],
            "dataType":0
		},
		{
			"seriseType": "wellSymbol",
            "fields":["jh","jm","jb","x","y"],
            "dataType":0
		},
		{
			"seriseType": "wellTrace",
            "fields":["jh","jk_x","jk_y","jd_x","jd_y","a_x","a_y","b_x","b_y"],
            "dataType":0
		},
		{
			"seriseType": "miningIndex",
            "fields":["jh"],
            "dataType":0
		},
		{
		...
		{
			"seriseType": "symbolLine",
            "fields":["id","x","y","maxx","minx","maxy","miny"],
            "dataType":0
		},
		{
			"seriseType": "wellLabel",
            "fields":["jh","value"],
            "dataType":0
        },
        {
    ...
    */
    parseDs:function(params,callback){
        ...
        if(me.chart.tpl){
            ...
            //解析数据集
            var mapping = null;
            me.seriesMapping.forEach(function(item){
                if(item.seriseType ===params.subType ){
                    mapping = item;
                }
            })
            if(mapping){
  ...
  */
   ...
   *@param {callbak} function 回调函数
   */
    _parseGeologyAlgorithmServData: function (url, mapping, params, callbak) {
        var me = this;
        var _url = hwchartsPATH + 'data/' + mapping.type + '.json';
  ...
  */
    ...
    */
    _getData: function (mapping, callbak) {
        ...
        // if(!dsopts){
        //     alert(mapping.seriesName +'数据未配置')
        //     return;
        // }
        ...
        if(dataType === 0){
            // 数据访问服务的数据
            me._parseDasData(url, mapping, mapping.params, callbak);

        }else if(dataType === 1){
            // 图形算法服务
            me._parseGeologyAlgorithmServData(url, mapping,  mapping.params, callbak);
        }else if(dataType === 2){
            // 数据解析服务
            me._parseDataParseServData(url, mapping,  mapping.params, callbak);
        }
    },
     /**
    *@desc 获取json
    ...
    */
    parseColumnIndexData:function(config,data,fields){
        var me = this;
        var orifields = ['ly','ls','lq','cy','cs','cq'];
        if(data.constructor == Array){
            var arr= [];
            data.forEach(function(item){
                var obj = {
                ...
                for(var i = 0;i<config.length;i++){
                    var v  = config[i];
                    obj.value.push(item[orifields[i]||orifields[i-6]]);
                    obj.name.push(v.name);
                    obj.color.push(v.color);
                    obj.company.push(v.company)
                    obj.shortName.push(v.shortName)
