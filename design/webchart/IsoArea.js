/******************************************************************
 ** 文件名:IsoArea.js 专业图形组件-设计模式--井位图
 ...
 ** 版 本:1.1
 ******************************************************************/
Ext.define('vmd.d.webchart.IsoArea',{
	extend:'vmd.base.WebChart',
	xtype:'vmd.d.webchart.IsoArea',
	requires:[
	...
	onRender: function(ct, position) {
		...
		// 是否存在内存对象
		var mgr=vmd.d.webchart.IsoArea.Mgr;
		...
		if(tempObj&&tempObj.tpl){
			...
			if(me.path){
				...
				var path = '/design/webchart/templates/isoarea.json';
			}
	...
	graphFit:function(w, h) {	
		...
		// 缩放比例
		var zoomFactor = h / frameHei;		
		var opt = me.chart.getOption();
		// 设置geo的属性
		for(var i = 0; i < opt.geo.length; i++)
		{
			opt.geo[i].zoom = zoomFactor;
		}
		me.chart.setOption({geo: opt.geo});
		// me.chart.setOption(opt);
    },
     ...
     * **/
		...
		var fileName = me.viewerNode.id+'isoarea.json';
	...
	vmd.d.webchart.IsoArea.Mgr= new Ext.util.MixedCollection();
})
