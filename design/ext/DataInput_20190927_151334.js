...
Ext.define('ide.ext.DataInput', {
	...
	setType: function(info, report, styleInfo, colset) {
		...
		for (var i = 0; i < info.settings.fields.length; i++) {
			...
			if (sets) {
				if (colset) {
					var cellInfo = report.main.body.sections[0].data[Math.floor(i / colset)].cells[(kk * 3 + 1) % (3 * colset)];
				} else {
					var cellInfo = report.main.body.sections[0].data[0].cells[i];
				}
				kk++
				switch (sets.type) {
					case "text":
					...
					case "checkbox":
						...
						checkBox.bindsource = {};
						checkBox.bindsource.tablename = sets.datas.dataSet
						checkBox.bindsource.valuecolumn = sets.datas.saveField
						checkBox.bindsource.showcolumn = sets.datas.displayField
						checkBox.bindsource.condition = sets.datas.filter
					...
					case "radioButton":
						...
						radioGroup.bindsource.tablename = sets.datas.dataSet
						radioGroup.bindsource.valuecolumn = sets.datas.saveField
						radioGroup.bindsource.showcolumn = sets.datas.displayField
					...
					case "comboGrid":
						...
						ddg.bindsource = {};
						ddg.bindsource.tablename = sets.datas.dataSet
						ddg.bindsource.valuecolumn = sets.datas.saveField
						ddg.bindsource.showcolumn = sets.datas.displayField
						// 
						//转化为单元格
						var selected = info.settings.fields[i].fieldsConfig.selected || [];
						var hideNum = 0;
								...
								for (var n = 0; n < selected.length; n++) {
									...
									if (selected[n].cellid == info.settings.fields[t].dictionary.name) {
										...
										if (colset) {
											...
											selected[n].cellid = (numToEng(t - hideNum) + 2).toString()
										}
					...
					case "comboTree":
						...
						ddt.bindsource = {};
						ddt.bindsource.tablename = sets.datas.dataSet
						ddt.bindsource.valuecolumn = sets.datas.valueField
						ddt.bindsource.showcolumn = sets.datas.displayField
						ddt.bindsource.parentcolumn = sets.datas.parentField
						ddt.bindsource.nodecolumn = sets.datas.leafValue
						ddt.bindsource.rootvalue = sets.datas.rootValue
						ddt.bindsource.roottext = sets.datas.rootText
					...
					case "combo":
						...
						comboBox.bindsource = {};
						comboBox.bindsource.tablename = sets.datas.dataSet;
						comboBox.bindsource.valuecolumn = sets.datas.saveField;
						comboBox.bindsource.showcolumn = sets.datas.displayField;
						comboBox.bindsource.condition = sets.datas.filter;
		...
		for (var i = startrow; i < endrow; i++) {
			...
			for (var n = startcol; n < endcol; n++) {
				if (i == 0) {
					...
					cells.push(
						this.cell('g0', (info.settings.fields[n].dictionary.cname == '') ? info.settings.fields[n].dictionary.name + "<br>" + unit : info.settings.fields[n].dictionary.cname + "<br>" + unit, info.settings.fields[n].dictionary, opts)
					)
				}
				if (i == 1) {
					opts.type2 = 'data';
					cells.push(
						this.cell('g1', sName + '.' + info.settings.fields[n].dictionary.name, info.settings.fields[n].dictionary, opts)
					)
				}
			}
			temp.push({
			...
			for (var n = 0; n < colset; n++) {
				...
				if (isHide) {
					...
					cells.push(this.cell('f0', (info.settings.fields[k].dictionary.cname == '') ? info.settings.fields[k].dictionary.name : info.settings.fields[k].dictionary.cname,
						info.settings.fields[k].dictionary, opts))
					cells.push(this.cell('f1', sName + '.' + info.settings.fields[k].dictionary.name, info.settings.fields[k].dictionary, opts))
					cells.push(this.cell('f2', '', info.settings.fields[k].dictionary, opts))
				}
			}
			temp.push({
				i: cells
