...
Ext.define("vmd.comp.Report", {
	...
	onRender: function(ct, position) {
		...
		if (!vmdreport.el) {
			...
			vmdreport.hwReport.enableMultiselect(true);
			// 非常规模式设置加载条数
			...
			if (vmdreport.isServer == undefined || vmdreport.isServer || !Ext.isIE) {
				...
				vmd.ajax({
					...
					success: function(res) {
						...
						vmd.isCheckFileExist("/" +vmdreport.configPath,function(resultInfo){
	...
	setColumnHidden:function(ind, state){
		this.hwReport.grid.setColumnHiddenExp(ind, state);
	},
	// 获取值
