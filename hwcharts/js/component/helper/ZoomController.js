Vmd.define('hwchart.component.helper.ZoomController', {
    requires: [
        'hwchart.component.helper.interactionMutex',
    ...
    var interactionMutex = hwchart.component.helper.interactionMutex;
    ...
    function createCover(controller, brushOption) {
        if (!coverRenderers[brushOption.brushType]) return
    ...
    function resetCursor(controller, e) {
        ...
        if (controller._brushType) { // If active
            ...
            var covers = controller._covers;
            var inCover;
            var brushKey = controller._brushKey;//zoomIn,zoomOut,restore
            for (var i = 0; i < covers.length; i++) {
                if (covers[i]&&coverRenderers[covers[i].__brushOption.brushType].contain(covers[i], x, y)) {
                    inCover = true;
                    break;
                }
            }
            ...
            if (!inCover) {
                if (panels) { // Brush on panels
                    each(panels, function (panel) {
                        if (panel.contain(x, y)) {
                            cursor = brushKey == 'zoomIn' ? 'zoom-in' : brushKey == 'zoomOut' ? 'zoom-out' : cursor;
    ...
    var mouseHandlers = {
        ...
        mousedown: function (e) {
            if (this._brushType == 'none') {
                if (e.target.__title) {
                    preventDefault(e);
                } else {
                   
                    if (this._brushKey == 'resotre') return;
                    trigger(this, {
                        brushKey: this._brushKey,
        ...
        mousemove: function (e) {
            ...
            if (e.target && e.target.__title) {
                this._zr.setCursorStyle('pointer');
                return
            }
            // set Cursor
            resetCursor(this, e);
        ...
        contextmenu: function (e) {
            ...
            //contextMenu(this, e.offsetX, e.offsetY);
        }
        // FIXME
    ...
    function contextMenu(controller, x, y) {
        if (typeof dhtmlx != 'undefined') {

            if (!controller._menu) {
                controller._menu = new dhtmlXMenuObject();
                controller._menu.renderAsContextMenu();
                // 添加菜单项
                controller._menu.addNewChild(controller._menu.topId, 1, "drag", '恢复拖动');

                // 菜单项添加点击事件
                controller._menu.attachEvent("onClick", function (id) {
                    if (id === "drag") {
                        //off 事件
                        doDisableBrush(controller);
                        //resotre 鼠标形状
                        controller.api.dispatchAction({
                            type: 'takeGlobalCursor'
                        });
                    }
            ...
            // 显示菜单
            controller._menu.showContextMenu(x + 10, y + 10);
        }
    }
    ...
    hwchart.component.helper.ZoomController = BrushController;
})