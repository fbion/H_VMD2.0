...
KISBPM.TOOLBAR = {
    ACTIONS: {
        ...
        saveModel: function (services) {
            ...
            // 2018.01.06还原鼠标默认状态
            services.$scope.editor.setMouseDragState(0);
        },
        ...
        addBendPoint: function (services) {
            ...
            // 2018.01.05：工具条流程线添加折弯点与流程绘图的关联
        ...
        // 2018.01.05:漫游
        mouseWander: function (services) {

            if (!services || !services.$scope || !services.$scope.editor || !services.$scope.editor.mouseWander) {
                return;
            }
            services.$scope.editor.mouseWander();
            document.body.style.cursor = 'url(editor-app/images/hand.png),auto';
        },

        // 2018.01.05:正常
        mouseNormal: function (services) {
            
            if (!services || !services.$scope || !services.$scope.editor || !services.$scope.editor.mouseNormal)
            {
                return;
            }
            services.$scope.editor.mouseNormal();
            document.body.style.cursor = 'default';
        },
    ...
    $scope.save = function (successCallback) {
        ...
        $http({    method: 'PUT',
            ...
            headers: {'Accept': 'application/json',
                ...
                'token': $rootScope.strOAuthId,
                'einfo':$rootScope.streinfo,
                'info':$rootScope.strinfo
                //'token': '{f0784bf4-1b44-4266-991f-bc7996946364}'
            },
            ...
            .success(function (data, status, headers, config) { // 2017.11.25:保存成功
                ...
                // 2017.11.25 sgw:调用VMD2.0集成的工作流页面，用于更新外部框架工作流节点是否发布的状态
                if(parent)
                {
                    if(parent.updateDeploymentState)
                    {
                        parent.updateDeploymentState();
                    }
                }
            })
