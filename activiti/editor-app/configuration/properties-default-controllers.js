...
var KisBpmStringPropertyCtrl = [ '$scope', function ($scope) {
    ...
    $scope.inputBlurred = function() { // 2017.09.08属性修改实时响应到图形上
...
var KisBpmTextPropertyPopupCtrlExt = ['$scope', function ($scope) {
    // 添加监听判断是否有值，有值就执行
    $scope.$watch('$parent.selectedItem', function (newValue, oldValue) {
        ...
        if (newValue) {
            ...
            for (var i = 0; i < newValue.properties.length; i++) {
                if (newValue.properties[i].key.toString() == "oryx-documentation") {
