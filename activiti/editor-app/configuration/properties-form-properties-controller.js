...
var KisBpmFormPropertiesPopupCtrlExt = ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {
    
    // 添加监听判断是否有值，有值就执行
    $scope.$watch('$parent.selectedItem', function (newValue, oldValue) {

        if (newValue) {
            //
            /****/
            for (var i = 0; i < newValue.properties.length; i++) {
                if (newValue.properties[i].key.toString() == "oryx-formproperties") {
                    $scope.property = newValue.properties[i];
                    break;
                }
            }
            if (!$scope.property) {
            ...
            //$scope.property = newValue.properties[5];

            // Put json representing form properties on scope
            if ($scope.property && $scope.property.value !== undefined && $scope.property.value !== null
                && $scope.property.value.formProperties !== undefined
                && $scope.property.value.formProperties !== null) {
                // Note that we clone the json object rather then setting it directly,
                // this to cope with the fact that the user can click the cancel button and no changes should have happended
