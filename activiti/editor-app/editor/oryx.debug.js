...
ORYX.Core.SVG.SVGShape = Clazz.extend({
    ...
    init: function () {
        ...
        if (ORYX.Editor.checkClassType(this.element, SVGRectElement) || ORYX.Editor.checkClassType(this.element, SVGImageElement)) {
            ...
            this.element.setAttributeNS(null, 'stroke', "#000000");
            this.element.setAttributeNS(null, 'stroke-width', "1");
            this.element.setAttributeNS(null, 'fill', "#ffffff");
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
     ...
     *
     */
    construct: function (source, modelMetaData, editorId, idOAuth, einfo, info) {
        arguments.callee.$.construct.apply(this, arguments);
        if (!source) {
        ...
        if (ORYX.CONFIG.BACKEND_SWITCH) {
            ...
            // 2017.12.25：从服务器请求节点画成什么样子
            // ACTIVITI.CONFIG.contextRoot + '/editor/stencilset?version=' + Date.now()
            // 这是请求服务器上的stencilset.json文件，这个文件描述了对象画成什么样子，请求回来的值设置了this._jsonObject对象
            new Ajax.Request(ACTIVITI.CONFIG.contextRoot + '/editor/stencilset?version=' + Date.now(), {
                asynchronous: false,
                method: 'get',
                requestHeaders: ['token', idOAuth, 'einfo', einfo, 'info', info],
                //requestHeaders: ['token', '{f0784bf4-1b44-4266-991f-bc7996946364}'],
    ...
    // 2017.12.25:绘图数据是从response中传过来的
    ...
    _init: function (response) {
        ...
        // init each stencil
        // 2017.12.25：所有节点画成什么样子都在this._jsonObject.stencils数组元素的view属性中
 ...
 */
ORYX.Core.StencilSet.loadStencilSet = function (url, modelMetaData, editorId, idOAuth, einfo, info) {
    ...
    //if(!stencilSet) {
    //load stencil set
    stencilSet = new ORYX.Core.StencilSet.StencilSet(url, modelMetaData, editorId, idOAuth, einfo, info);
...
ORYX.Core.UIObject = {
    ...
    _delegateEvent: function (event) {
        ...
        // 2017.10.28 check fault 引起鼠标悬浮无法选中的问题
        //if (2 == ORYX.Editor.modeShow)
        //{
        //    if (event.type != ORYX.CONFIG.EVENT_MOUSEDOWN) {
        //        return;
        //    }
        //}
    ...
    add: function (uiObject, index, silent) {
        ...
        if (uiObject instanceof ORYX.Core.UIObject) {
            if (!(this.children.member(uiObject))) {
                ...
                //2017.09.04流程线箭头
                var markerEnd = jQuery('defs').find('[id$=end]').find('path');
                if (markerEnd) {
                    var drawPath = markerEnd.attr('d');
                    if (drawPath && drawPath.length > 1) {
                        if (drawPath[drawPath.length - 1] == 'z' || drawPath[drawPath.length - 1] == 'Z') {
                            drawPath = drawPath.substring(0, drawPath.length - 1);
                        }

                        // 调整箭头长度
                        // M 0 1 L 15 6 L 0 11
                        var arrCoor = drawPath.split(' ');
                        if (arrCoor.length == 9) {
                            var nOffX = 1;
                            arrCoor[1] = '5';
                            arrCoor[7] = '5';
                            //arrCoor[1] = (parseInt(arrCoor[1]) + nOffX).toString();
                            //arrCoor[7] = (parseInt(arrCoor[7]) + nOffX).toString();

                            drawPath = arrCoor.join(' ');
                        }
...
ORYX.Editor = {
    ...
    construct: function (config, Oathconfig, arrSelectedNodeId) {
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // 显示模式：0：设计器模式，1：监视器模式；2：支持可视化模式；3：支持可视化节点多选模式
        this.modeShow = 0;
        this.arrSelectedNodeId = arrSelectedNodeId;
        this.ifVisualDblClickSendMessage = false;
        this.jsonHighlight = null;
        this.idOAuth = Oathconfig.idOAuth;
        this.einfo = Oathconfig.einfo;
        this.info = Oathconfig.info;
        this.mouseDragState = 0; // 2018.01.03:标识鼠标状态，0：正常；1：漫游
        this.sizeTitle = 24; // 2018.01.08:标题字体大小
        this.sizeTitleOrg = 24; // 2018.01.08:标题字体大小
        ...
        if (ORYX.CONFIG.BACKEND_SWITCH) {
            ...
            ORYX.Core.StencilSet.loadStencilSet(ssUrl, this.modelMetaData, this.id, this.idOAuth, this.einfo, this.info);
        } else {
        ...
        window.setTimeout(function () {
            var arrShapes = this.loadSerialized(model, true); // Request the meta data as well
            this.getCanvas().update();
            loadContentFinished = true;
            initFinished();

            this._setPluginsMode();
            this._domOperate(arrShapes, arrSelectedNodeId);
            if (parent) {
                // 20190118:流程加载完成后，设置节点是否选中
                if (3 == this.modeShow) // 可视化节点多选模式
                {
                    //var arrTmp = ['R2', 'R3', 'R1'];
                    //parent.arrVisualSelectedNodes = arrTmp;
                    this._setVisualSelectedNodes(parent.arrVisualSelectedNodes, arrShapes);
                }

                // 2017.12.08：流程加载完成后，调用通知外部调用框架
                if (parent.workflowPageLoaded) {
                    parent.workflowPageLoaded();
                }
            }
        }.bind(this), 200);
    },

    _setVisualSelectedNodes: function (arrVisualSelectedNodes, arrShapes) {

        if (3 != this.modeShow) // 可视化节点多选模式
        {
            return;
        }
        if (!arrVisualSelectedNodes)
        {
            return;
        }
        for (var i = 0; i < arrVisualSelectedNodes.length; i++) {
            for (var j = 0; j < arrShapes.length; j++) {
                var strOverrideId = arrShapes[j].properties["oryx-overrideid"];
                if (strOverrideId == arrVisualSelectedNodes[i]) {
                    var itemSel = arrShapes[j];
                    if (itemSel.selectedMark) {
                        itemSel.selectedMark.show();
                    }
                    var itemTmp = [itemSel];
                    this.selectionMarks.push(itemTmp);
                    break;
                }
            }
        }
    ...
    _setPluginsMode: function () {
        for (var i = 0; i < this.loadedPlugins.length; i++) {
            // 2017.10.28 check fault
            //if (this.loadedPlugins[i].type == 'ORYX.Plugins.DragDocker')
            if (this.loadedPlugins[i].type == 'ORYX.Plugins.DragDocker' || this.loadedPlugins[i].type == 'ORYX.Plugins.DragDropResize'
                 || this.loadedPlugins[i].type == 'ORYX.Plugins.RenameShapes') {
                this.loadedPlugins[i].modeShow = this.modeShow;
                //break;
            }
        }
    },

    _domOperate: function (arrShapes, arrSelectedNodeId) {

        // 2017.12.14：绘图区域显示滚动条
        //this._changeCanvasRegion();
        //if (0 == this.modeShow)
        //{
        //}
        // 监视器模式
        if (this.modeShow == 1) {
            ...
            if (this.jsonHighlight) {
                if (this.jsonHighlight.activities) {
                    for (var i = 0; i < this.jsonHighlight.activities.length; i++) {
                        // 加载完成后根据ID获取svg节点，设置高亮状态
                        jQuery('#svg-' + this.jsonHighlight.activities[i]).find('[id*=bg_frame]').attr({
                            'stroke': '#d60000',
                            'stroke-width': '3'
                        });
                    }
                ...
                // 流程线
                if (this.jsonHighlight.flows) {
                    for (var i = 0; i < this.jsonHighlight.flows.length; i++) {
                        // 加载完成后根据ID获取svg节点，设置高亮状态
                        var nodeResult = jQuery('#svg-' + this.jsonHighlight.flows[i]).find('[id$=_1]');
                        nodeResult.attr({
                            'stroke': '#d60000'
                        });

                        var markerendUrl = nodeResult.attr('marker-end');
                        var markerendId = markerendUrl.substring(5, markerendUrl.length - 1);

                        //流程线箭头
                        var markerend = jQuery('defs').find('[id*=' + markerendId + ']').find('path').attr({
                            'stroke': '#d60000',
                            'fill': '#d60000'
                        });
                    }
        ...
        else if (this.modeShow == 2) {
            this._addSelectedNodes(arrShapes, arrSelectedNodeId);
            //this._markSelection();
        }
        ...
        // 2018.01.02:设置流程图的标题
        this._setWorkflowTitle(this.modelMetaData.name);

        // 2018.01.04:属性面板Tab页自适应
        //var arrTabbarNodes = document.getElementsByClassName("dhxtabbar_cont");
        //this._resetNodesStyle(arrTabbarNodes);
    },

    _resetNodesStyle: function (arrNodes) {
        ...
        for (var i = 0; i < arrNodes.length; i++) {
            if (arrNodes[i]) {
                if (arrNodes[i].style) {
                    arrNodes[i].style.position = "relative";
                    arrNodes[i].style.height = "auto";
                    arrNodes[i].style.top = "0px";
                }
                this._resetNodesStyle(arrNodes[i].childNodes);
            }
        }
    },

    // 2018.01.02：设置工作流的标题
    _setWorkflowTitle: function (title) {
        if (!title) {
            return;
        }
        var arrSvgCanvas = document.getElementsByClassName("ORYX_Editor");
        if (arrSvgCanvas) {
            if (arrSvgCanvas.length > 0) {
                if (arrSvgCanvas[0].childNodes && arrSvgCanvas[0].childNodes.length > 0) {
                    var nFontsize = this.sizeTitle;
                    var nWidth = 0;
                    if (arrSvgCanvas[0].childNodes[0].width) {
                        if (arrSvgCanvas[0].childNodes[0].width.baseVal) {

                            if (arrSvgCanvas[0].childNodes[0].width.baseVal.value) {

                                nWidth = arrSvgCanvas[0].childNodes[0].width.baseVal.value;
                            }
                        }
                    }
                    var nPosX = nWidth / 2;
                    var nPosY = 10;
                    var textTitle = this._makeSVG('text', {
                        x: nPosX, y: nFontsize + 5, stroke: 'black', 'stroke-width': 1, fill: 'black',
                        id: 'workflowTitle',
                        style: 'font:' + nFontsize + 'px simsun;'
                    });
                    textTitle.textContent = title;
                    arrSvgCanvas[0].childNodes[0].appendChild(textTitle);
					var nPosXTmp = nPosX - textTitle.clientWidth / 2;
                    if (nPosXTmp < 0)
                    {
                        nPosXTmp = 0;
                    }
                    textTitle.setAttribute('x', nPosXTmp);
                    //textTitle.setAttribute('x', nPosX - textTitle.clientWidth / 2);
                }
            }
        }
    },
    ...
    _changeCanvasRegion: function () {
        ...
        if (arrSvgCanvas) {
            ...
            if (objPaletteHelpWrapper) {
                nPaletteHelpWrapper = objPaletteHelpWrapper.offsetHeight;
            }
            for (var i = 0; i < arrSvgCanvas.length; i++) {
                arrSvgCanvas[i].style.width = document.body.clientWidth + "px";
                arrSvgCanvas[i].style.height = document.body.clientHeight - nToolbarHei - nPaletteHelpWrapper + "px";
                arrSvgCanvas[i].style.overflow = "auto"; // 2017.12.14:设置绘图区域可以显示滚动条
            }
            //document.getElementsByClassName("ORYX_Editor").style.width = document.body.clientWidth + "px";
            //document.getElementsByClassName("ORYX_Editor").style.clientHeight = document.body.clientHeight + "px";
        }
        // jQuery操作DOM
        //var arrSvgCanvas = jQuery(".ORYX_Editor");
        //if (arrSvgCanvas)
        //{
        //    if (arrSvgCanvas.length > 0) {

        //        arrSvgCanvas[0].style.width = document.body.clientWidth + "px";
        //        arrSvgCanvas[0].style.height = document.body.clientHeight - 300 + "px";
        //        //jQuery(".ORYX_Editor")[0].style.width = document.body.clientWidth + "px";
        //        //jQuery(".ORYX_Editor")[0].style.height = document.body.clientHeight - 300 + "px";
        //    }
        //}
    },

    _addSelectedNodes: function (arrShapes, arrSelectedNodeId) {
        if (!arrSelectedNodeId || !arrShapes) {
            return;
        }
        for (var i = 0; i < arrSelectedNodeId.length; i++) {
            var strNodeId = arrSelectedNodeId[i];
            // oryx-overrideid
            for (var j = 0; j < arrShapes.length; j++) {
                if (arrShapes[j].properties) {
                    var strOverrideId = arrShapes[j].properties["oryx-overrideid"];
                    //var strOverrideId1 = arrShapes[j].properties.oryx-overrideid;
                    if (strOverrideId == strNodeId) {

                        var newSelection = this.selection.clone();
                        newSelection.push(arrShapes[j]);
        ...
        //            if (model.childShapes[j].properties) {
        //                if (model.childShapes[j].properties.overrideid
        //                    == strNodeId) {

        //                    var newSelection = this.selection.clone();
        //                    newSelection.push(model.childShapes[j]);
        //                    this.setSelection(newSelection);
    ...
    _initEventListener: function () {
        ...
        this.DOMEventListeners[ORYX.CONFIG.EVENT_DBLCLICK] = [];
    ...
    _generateGUI: function () {
        ...
        var nCanvasWidth = ORYX.CONFIG.CANVAS_WIDTH;
        nCanvasWidth = this._getCanvasWid();
    ...
    _createCanvas: function (stencilType, canvasConfig) {
        ...
        //div.style.overflow = "auto"; // 2017.12.14:设置绘图区域可以显示滚动条
    ...
    _getCanvasWid: function () {
        ...
        var nCanvasWidth = ORYX.CONFIG.CANVAS_WIDTH;
        var nCanvasWidthTmp = document.body.clientWidth - nPaletteHelpWrapperWid - 17;
        if (nCanvasWidth < nCanvasWidthTmp) {
            nCanvasWidth = nCanvasWidthTmp;
        }
        return nCanvasWidth;
    },
     ...
     * 2018.01.05:绑定外部可以访问的方法，返回facade对象
	 */
    _getPluginFacade: function () {
        ...
        if (!(this._pluginFacade))
            ...
            this._pluginFacade = {
                ...
                getMouseDragState: this.getMouseDragState.bind(this),// 2018.01.05：扩展绑定的方法
                zoomTitle: this.zoomTitle.bind(this),// 2018.01.08：标题的缩放
    ...
    /**
     * 2018.01.12:扩展getJSON不删除toJSON方法
     */
    getJSONExt: function () {
        //delete Array.prototype.toJSON;
        var canvasJSON = this.getCanvas().toJSON();
	...
	*/
    ...
    loadSerialized: function (model, requestMeta) {
        ...
        //var nodeResult = jQuery('#svg-deptLeaderAudit').find('[id*=bg_frame]');
        //nodeResult.attr('stroke', '#ff0000');

        return shapes;
    },

    // 根据ID递归查找绘图节点（rect、circle、path等）
    //getElementById: function(idElem, arrNodes){

    //    for(var i = 0; i < arrNodes.length; i++)
    //    {
    //        var tmp = arrNodes[i].id;
    //        if (tmp)
    //        {
    //            if (tmp.indexOf(idElem) > -1) {
    //                return arrNodes[i];
    //            }
    //        }
    //        var nodeResult = this.getElementById(idElem, arrNodes[i].childNodes);
    //        if(nodeResult)
    //        {
    //            return nodeResult;
    //        }
	...
	*/
    ...
    loadStencilSet: function (source) {
        try {
            ORYX.Core.StencilSet.loadStencilSet(source, this.modelMetaData, this.id, this.idOAuth);
    ...
    setSelection: function (elements, subSelectionElement, force) {
        ...
        //this.selectionMarks.push(elements);
        ...
        // 2017.11.16：注册双击事件，否则在双击事件中无法获取的event.elements为空
        //this.handleEvents({ type: ORYX.CONFIG.EVENT_DBLCLICK, elements: elements, subSelection: subSelectionElement, force: !!force });
    ...
    getMouseDragState: function () {
        return this.mouseDragState;
    },

    // 2018.01.08：缩放标题
    zoomTitle: function (fZoomLevel) {

        this.sizeTitle = this.sizeTitleOrg * fZoomLevel;
        
        var divTitle = document.getElementById("workflowTitle");
        if (divTitle)
        {
            divTitle.style.fontSize = this.sizeTitle + 'px';
            ...
            if (arrSvgCanvas) {
                if (arrSvgCanvas.length > 0) {
                    if (arrSvgCanvas[0].childNodes && arrSvgCanvas[0].childNodes.length > 0) {
                        ...
                        var nPosX = nWidth / 2 - divTitle.clientWidth / 2;
                        var nPosY = this.sizeTitle + 5;
                        divTitle.setAttribute('x', nPosX);
                        divTitle.setAttribute('y', nPosY);
                    }
                }
            }
        }
	...
	*/
    ...
    handleEvents: function (event, uiObj) {
        ...
        if (this.modeShow == 1) {
        ...
        else if (this.modeShow == 2) { // 2017.11.17
            var elements = event.elements;
            if (elements && elements.length > 0) {

                if (elements[0].getStencil()) {
                    var strNodeType = elements[0].getStencil().idWithoutNs();
            ...
            if (event.type == ORYX.CONFIG.EVENT_MOUSEMOVE || event.type == ORYX.CONFIG.EVENT_MOUSEUP
                || event.type == ORYX.CONFIG.EVENT_MOUSEOUT) {
                this.ifVisualDblClickSendMessage = false;
                return;
            }
        }

        if (1 != this.mouseDragState) {
            ...
            switch (event.type) {
                ...
                case ORYX.CONFIG.EVENT_MOUSEOUT:
                    ...
                    break;
                case ORYX.CONFIG.EVENT_DBLCLICK:
                    // 2017.11.16：注册双击事件，否则在双击事件中无法获取的event.elements为空
                    //this.handleEvents({ type: ORYX.CONFIG.EVENT_DBLCLICK, elements: elements, subSelection: subSelectionElement, force: !!force });
            ...
            // 设计器
            // 2017.10.28 check fault
            if (this.modeShow == 0 || (this.modeShow == 2 && (event.type == ORYX.CONFIG.EVENT_MOUSEDOWN || event.type == ORYX.CONFIG.EVENT_DBLCLICK))) {
                //if (this.modeShow == 0) {
                if (!this._queueRunning) {
                    this.ifVisualDblClickSendMessage = true;
                    this._executeEvents();
                }
            }
        }
        ...
        {
            if (event.type != ORYX.CONFIG.EVENT_MOUSEOVER) {
                /* Force execution if necessary. Used while handle Layout-Callbacks. */
                if (event.forceExecution) {
                    this._executeEventImmediately({ event: event, arg: uiObj });
                } else {
        ...
        // 2018.01.05:控制鼠标拖拽时的光标形状；
        if (1 == this.mouseDragState) {
            document.body.style.cursor = 'url(editor-app/images/hand.png),auto';
            //return;
        }
        //else {
    ...
    _markSelection: function () {
        ...
        // 2019.01.16:设置节点选中状态
        for (var j = 0; j < this.selectionMarks.length; j++) {
            var itemSel = this.selectionMarks[j];
            for (var i = 0; i < itemSel.length; i++) {
                if (itemSel[i].selectedMark) {
                    itemSel[i].selectedMark.show();
                }
            }
        }
        //if (this.selection) {
            //for (var i = 0; i < this.selection.length; i++) {
            //    if (this.selection[i].selectedMark) {
            //        this.selection[i].selectedMark.show();
            //    }
            //}
        //}
    ...
    //_sendMultiNodeInfo: function (ifDblClick) {	    
    ...
    //    strResultInfo += "'modelName':'" + strProcessName + "',"; // 模板名称
    //    strResultInfo += "'variants':["; // 模板ID(ModelId)

    //    var nCnt = this.selection.length;
    //    for (var i = 0; i < nCnt; i++) {
    //        strResultInfo += "{";

    //        var selectedShape = this.selection[i];
    ...
    //                            // 节点变量
    //                            var taskresultStr = "";
    //                            if (typeof (formProperties.formProperties) != "undefined" && formProperties.formProperties != null) {
    ...
    //                                        if (typeof (formPropertiesStart.formProperties) != "undefined" && formPropertiesStart.formProperties != null) {
    ...
    //    alert(strResultInfo);
    //    // 2017.12.06:调用父窗口页面定义的方法，传递变量信息
    //    //if (parent) {
    //    //    if (ifDblClick) {
    ...
    _handleMouseDown: function (event, uiObj) {
        ...
        var elementController = uiObj; // 2017.12.07:获取到的点击对象

        // 可视化应用
        // 2017.12.07:屏蔽按下Ctrl键时选中除开始节点和任务节点之外的其他节点
        if (this.modeShow == 2) {
            if (elementController) {
                if (elementController.getStencil()) {
                    var strNodeType = elementController.getStencil().idWithoutNs();
                    if (!(strNodeType == "StartNoneEvent" || strNodeType == "StartTimerEvent"
                    || strNodeType == "StartSignalEvent" || strNodeType == "StartMessageEvent"
                    || strNodeType == "StartErrorEvent"
                    || strNodeType == "UserTask" || strNodeType == "ServiceTask"
                    || strNodeType == "ScriptTask" || strNodeType == "BusinessRule"
                    || strNodeType == "ReceiveTask" || strNodeType == "ManualTask"
                    || strNodeType == "MailTask" || strNodeType == "CamelTask"
                    || strNodeType == "MuleTask" || strNodeType == "SendTask" || strNodeType == "BPMNDiagram")) {
                        return;
                    }
                }
            }
        ...
        // 2019.01.16:如果节点可选择，且已经选中，就取消原来的选中效果，即从已选中节点数组中删除此项
        if (currentIsSelectable && !modifierKeyPressed) {
            var currentElement = [elementController];
            this.visualNodeMultiSelect(currentElement);
        }
    ...
    */
    ...
    visualNodeMultiSelect: function (currentElement) {

        // 可视化节点多选模式
        if (this.modeShow != 3 || !currentElement) {
            return;
        }
        // 2019.01.16:如果节点可选择，且已经选中，就取消原来的选中效果，即从已选中节点数组中删除此项
        var bExisted = false;
        for (var j = 0; j < this.selectionMarks.length; j++) {
            ...
            var itemSel = this.selectionMarks[j];
            if (itemSel.length > 0 && currentElement.length > 0) {
                if (itemSel[0].id == currentElement[0].id) {
                    if (itemSel[0].selectedMark) {
                        itemSel[0].selectedMark.hide();
                    }
                    bExisted = true;
                    // 已经选中的元素就取消选中
                    this.selectionMarks.splice(j, 1);  //从索引j开始删除1个
                    break;
                }
            }
        }
        if (!bExisted) {
            this.selectionMarks.push(currentElement);
        }

        this._markSelection();
    },

    setMouseDragState: function (nState) {
        //alert("mouseWander");
        this.mouseDragState = nState; // 2018.01.03:标识鼠标状态，0：正常；1：漫游
    },

    mouseWander: function (event) {
        //alert("mouseWander");
        this.mouseDragState = 1; // 2018.01.03:标识鼠标状态，0：正常；1：漫游
    },

    mouseNormal: function (event) {
        //alert("mouseNormal");
        this.mouseDragState = 0; // 2018.01.03:标识鼠标状态，0：正常；1：漫游
    },
...
ORYX.Core.Node = {
    ...
    construct: function (options, stencil, facade) {
        ...
        // 2019.01.16:Visual模式下的节点选中标识（对勾）
        this.selectedMark = new ORYX.Plugins.SelectedMark(containerNode);
    ...
    _update: function () {
        ...
        if (this.isChanged) {
            ...
            this.selectedMark.resize(bounds);
    ...
    _init: function (svgDocument) {
        ...
        //this.deleteEventHandlers(svgNode.parentNode);
    ...
    //ORYX.Plugins.SelectedRectVisual = Clazz.extend({
    ...
    //            ['rect', {
    //                x: 0, y: 0, rx: 10, ry: 10,
    //                'stroke-width': 2, stroke: '#d60000', fill: 'none',
    //                'pointer-events': 'none'
    //            }]);
	...
	*/
    ...
    _init: function (svgDocument) {
        ...
        this._paths.each((function (path) {
            ...
            iPath.setAttributeNS(null, "stroke-width", 1);
...
ORYX.Plugins.View = {
    ...
    zoom: function (factor) {
        ...
        this.facade.zoomTitle(canvas.zoomLevel);
    },
...
ORYX.Plugins.DragDropResize = ORYX.Plugins.AbstractPlugin.extend({
    ...
    construct: function (facade) {
        ...
        this.facade = facade;
        this.modeShow = 0;
        this.bIfMouseDown = false; // 2018.01.03:标识鼠标是否按钮，用于处理鼠标漫游
        //this.mouseDragState = 0; // 2018.01.03:标识鼠标状态，0：正常；1：漫游
        this.mouseDownPos = { x: 0, y: 0 };	// 2018.01.04：鼠标按下点，用于处理漫游功能
        ...
        // 用于支持可视化定制中选中对象时的选中矩形
        this.selectedRectVisual = new ORYX.Plugins.SelectedRectVisual(containerNode);
        //this.highlightRect = new ORYX.Plugins.HighlightRect(containerNode);
    ...
    handleMouseDown: function (event, uiObj) {
        ...
        var mouseDragState = this.facade.getMouseDragState();
        if (1 == mouseDragState)// 2018.01.03:鼠标漫游状态
        {
            this.handleMouseDownWander(event, uiObj);
        }
        else {
            this.handleMouseDownNormal(event, uiObj);
        }

        return;
    },

    handleMouseDownWander: function (event, uiObj) {

        this.bIfMouseDown = true;// 2018.01.03:标识鼠标是否按钮，用于处理鼠标漫游
        document.body.style.cursor = 'url(editor-app/images/hand.png),auto';
        // 2018.01.08:开始漫游时，取消所有节点的选中
        //this.facade.setSelection([]);

        var objCanvas = this.facade.getCanvas();
        var elements = objCanvas.children;
        ...
        this.mouseDownPos = { x: eventX, y: eventY };	// 2018.01.04：鼠标按下点，用于处理漫游功能
    ...
    handleMouseDownNormal: function (event, uiObj) {
        // If the selection Bounds not intialized and the uiObj is not member of current selectio
        // then return
        if (!this.dragBounds || !this.currentShapes.member(uiObj) || !this.toMoveShapes.length) { return; };
    ...
    handleMouseUp: function (event) {
        //var mouseDragState = 1;
        var mouseDragState = this.facade.getMouseDragState();
        if (1 == mouseDragState)// 2018.01.03:鼠标漫游状态
        {
            this.handleMouseUpWander(event);
        }
        else {
            this.handleMouseUpNormal(event);
        }

        return;
    },

    handleMouseUpNormal: function (event) {
    ...
    handleMouseUpWander: function (event) {
        if (!this.bIfMouseDown) {
            return;
        }
        document.body.style.cursor = 'url(editor-app/images/hand.png),auto';
        ...
        var eventX = Event.pointerX(event);
        var eventY = Event.pointerY(event);
        var mouseUpPos = { x: eventX, y: eventY };	// 2018.01.04：鼠标弹起点，用于处理漫游功能
        if (this.mouseDownPos)
        {
            if (!(Math.abs(mouseUpPos.x - this.mouseDownPos.x) < 10 && Math.abs(mouseUpPos.y - this.mouseDownPos.y) < 10))
            {
                // Refresh all Shapes --> Set the new Bounds
                this.refreshSelectedShapesWander(mouseUpPos);
                this.facade.updateSelection();
	...
	*/
    handleMouseMove: function (event) {
        ...
        {
            document.body.style.cursor = 'url(editor-app/images/hand.png),auto';
            this.handleMouseMoveWander(event);
        }
        else {
            this.handleMouseMoveNormal(event);
        }
    ...
    handleMouseMoveNormal: function (event) {
    ...
    handleMouseMoveWander: function (event) {
        // If dragging is not enabled, go return
        if (!this.dragEnable) { return };
        // If Dragging is initialized
        if (this.dragIntialized)
	 ...
	 *
	 */
    refreshSelectedShapesWander: function (posMouseUp) {
        // If the selection bounds not initialized, return
        if (!this.dragBounds) { return }

        // Calculate the offset between the bounds and the old bounds
        //var upL = posMouseUp;
        //var oldUpL = this.mouseDownPos;
        //var offset = {
        //    x: (upL.x - oldUpL.x),
        //    y: (upL.y - oldUpL.y)
        //};
        ...
        //var offset = {
        //    x: upL.x - (oldUpL.x * this.faktorXY.x),
        //    y: upL.y - (oldUpL.y * this.faktorXY.y)
        //};
        //this.offSetPosition = {
        //    x: eventX - (upL.x * this.faktorXY.x),
        //    y: eventY - (upL.y * this.faktorXY.y)
    ...
    onSelectionChanged: function (event) {
        ...
        //var tmp = elements[0].getId();
        //tmp = elements[0].getStencil();
        ...
        //    var tmp1 = elements[0].getStencil();
        //    tmp1 = elements[0].getStencil().id();
        //    tmp1 = elements[0].getStencil().idWithoutNs();
        //}
        ...
        if (!elements || elements.length == 0) {
            ...
            if (this.modeShow == 2) {
                ...
                //var tmp = elements[0].getStencil().id();
                this.selectedRect.hide();
                // 2017.12.08：节点多选
                //this.selectedRectVisual.show();
                this.resizerSE.hide();
    ...
    resizeRectangle: function (bounds) {
        ...
        this.selectedRectVisual.resize(bounds);
        //this.highlightRect.resize(bounds);
    }
...
ORYX.Plugins.SelectedMark = Clazz.extend({
    ...
    construct: function (parentId) {
        ...
        this.node = ORYX.Editor.graft("http://www.w3.org/2000/svg", $(parentId),
					['g']);
        // 20190116:modeler-visual模式下，节点选中效果
        this.dashedArea = ORYX.Editor.graft("http://www.w3.org/2000/svg", this.node,
                            ['path',
							//{ "stroke-width": 3.0, "stroke": "#00CD00", "fill": "none", "d": "M-74,30 L-56,50 L-6,6", "line-captions": "none" }
							{ "stroke-width": 3.0, "stroke": "#d60000", "fill": "none", "d": "M-24,12 L-16,20 L-3,4", "line-captions": "none" }
							//{ "stroke-width": 3.0, "stroke": "#00CD00", "fill": "none", "d": "M-26,10 L-16,20 L2,-2", "line-captions": "none" }
                            //{ "stroke-width": 3.0, "stroke": "#16964D", "fill": "none", "d": "M-20,12 L-14,16 L-7,4", "line-captions": "none" }
                            //{ "stroke-width": 2.0, "stroke": "#019a01", "fill": "none", "d": "M-20,12 L-14,16 L-7,4", "line-captions": "none" }
                            //{ "stroke-width": 2.0, "stroke": "#d60000", "fill": "none", "d": "M-13,7 L-8,11 L-3,1", "line-captions": "none" }
                            // 
                            //{ "stroke-width": 2.0, "stroke": "#d60000", "fill": "none", "d": "M-14,8 L-8,12 L-2,0", "line-captions": "none" }
                            //{ "stroke-width": 2.0, "stroke": "#d60000", "fill": "none", "d": "M-12,8 L-8,12 L-2,2", "line-captions": "none" }
                            ]);
        ...
        //        [
        //            ['path',
        //            { "stroke-width": 2.0, "stroke": "#d60000", "fill": "none", "d": "M-18,8 L-12,12 L-4,0", "line-captions": "none" }
        //            ],
        //            ['rect',
        //                { x: 0, y: 0, rx: 10, ry: 10, 'stroke-width': 2, stroke: '#d60000', fill: 'none', 'pointer-events': 'none' }
        //            ]
        //        ]);
        //this.dashedArea = ORYX.Editor.graft("http://www.w3.org/2000/svg", this.node,

        //				['path',
        //					{ "stroke-width": 5.0, "stroke": "#d60000", "fill": "#d60000", "d": "M3,3 l0,-2.5 l7.5,0 l0,-2.5 l7.5,4.5 l-7.5,3.5 l0,-2.5 l-8,0", "line-captions": "round" }
        //				]);
    ...
    resize: function (bounds) {
        ...
        this.dashedArea.setAttributeNS(null, 'width', bounds.width() + 2 * padding);
        this.dashedArea.setAttributeNS(null, 'height', bounds.height() + 2 * padding);
        this.node.setAttributeNS(null, 'transform', "translate(" + (lwR.x - padding) + ", " + (upL.y - padding) + ")");
    }


});

//ORYX.Plugins.HighlightRect = Clazz.extend({
...
//			['rect', {
//			    x: 0, y: 0, rx: 10, ry: 10,
//				'stroke-width': 3, stroke: '#ff0000', fill: 'none',
//				'pointer-events': 'none'
//			}]);
...
ORYX.Plugins.DragDocker = Clazz.extend({
    ...
    construct: function (facade) {
        ...
        //
        //var strModeState = this.getUrlParameter('mode');
        //// 模式：0：设计器模式，1：监视器模式；2：支持可视化模式
    ...
    /**
	 * MouseOver Handler
	 * 2018.01.06:鼠标悬浮时在流程线上显示调整点
	 */
    handleMouseOver: function (event, uiObj) {
        // If there is aDocker, show this	
        //;
        //this.modeDesign;
        // 采用静态变量记录的方式（此处也可以通过getUrlParameter('mode');从URL中重新获取参数值判断状态（设计/监视））
        if (this.modeShow == 0) {
            //if (ORYX.Plugins.DragDocker.modeDesign) {
            if (!this.docker && uiObj instanceof ORYX.Core.Controls.Docker) {
                uiObj.show();
            } else if (!this.docker && uiObj instanceof ORYX.Core.Edge) {
                uiObj.dockers.each(function (docker) {
        ...
        else { // 2017.08.15鼠标悬浮时对象节点的圆角外框，流程线的浮动边线外框
        }
    },
    /**
	 * DockerDrag Handler
...
new function () {
    ...
    ORYX.Plugins.BPMN2_0 = {
        ...
        getLanes: function (shape, recursive) {
            ...
            var lanes = null;
            var childNodes = shape.getChildNodes(recursive || false);
            if (childNodes) {
                lanes = childNodes.findAll(function (node) {
                    var stencilTmp = node.getStencil();
                    if (stencilTmp) {
                        return (stencilTmp.id() === namespace + "Lane");
                    }
