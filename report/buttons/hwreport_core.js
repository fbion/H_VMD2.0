...
(function () {
    ...
    */
    ...
    function RptExpress(hwReport) {
         ...
         *  */
        this.calculate = function (exp, type, contextCell) {
            ...
            switch (paramType) {
                ...
                case "ds":
                    this.parse(exp);
                    return this.translateDs(exp, type, contextCell);
                    break;
                case "cell_approve":
                case "cell_uploader":
        ...
        */
         ...
         * type: 希望返回类型
         */
        this.translateDs = function (exp, type, contextCell) {
            var that = this;
    ...
    function RptPageSet(hwReport) {
        ...
        this.pageDirection = 1; //纸张方向 1表示纵向，0表示横向
        this.pageSize = "A4";
        this.position = "";
        ...
        this.setOptions = function (options) {
            ...
            this.set("pageDirection", options.pageDirection);//纸张方向 1表示纵向，0表示横向
        ...
        this.genPages = function (colWidths, rowHeights, title, header, datas) {
            ...
            var _headerHeight = rowHeights.slice(0, this.hwReport.fixedRowCount).reduce(function (total, num) { return total + parseInt(num) }, 0);
            
            var titlePages = _hasTitle ? this.genPagesData(colWidths,rowHeights.slice(0, 1), title,0, "title") : null;
            var headerPages = _hasHeader ? this.genPagesData(colWidths, rowHeights.slice(_hasTitle ? 1 : 0, this.hwReport.fixedRowCount), header, 0, "header") : null;
            var dataPages = this.genPagesData(colWidths, rowHeights.slice(this.hwReport.fixedRowCount), datas, _headerHeight, "data");
            ...
            for (var i = 0; i < datas.length; i++) {
                ...
                for (var j = 0; j < max_len; j += colspan_ind) {
                    ...
                    if (flag == "title") {
                        ...
                        if (this.hwReport.fixedRowCount > 0) {
                            if (!this.repeatHeader && pageStartRowIndex > 0 && accumHeight == 0 && rowHeights[i] > 0) {
                                posinfo = "c-top ";
                            }
                        }
                        else if (this.hwReport.fixedRowCount == 0 && accumHeight == 0 && rowHeights[i] > 0) {
                            posinfo = "c-top ";
                        }
                    }
                    ...
                    for (var k = 0; k < colSpan; k++) {
                        ...
                        if (!page) {
                            page = pages[curPageRow + "_" + curPageCol] = new Page(curPageRow, curPageCol);
                        }
                        //当前单元格是否已经放到分页中
                        if (!copytd._hasPaged) {
        ...
        this.genPagesDom = function (titlePages, headerPages, dataPages) {
            ...
            for (var i = 0; i < pages.length; i++) {
                ...
                if (!this.repeatHeader && page.row > 0) {
                    var pageDom = this.genPageDom(null, null, page);
                    container.appendChild(pageDom);
                }
                else {
                    var pageDom = this.genPageDom(titlePages && titlePages["0_" + page.col], headerPages && headerPages["0_" + page.col], page);
                    container.appendChild(pageDom);
                }
            }
            return container;
        ...
        */
        this.parse = function (pagejson, enattr) {
            ...
            this.set("pageBasic", pagejson[getAttrName("pageproperty", enattr)]);
        }

        function Page(row, col) {
            this.row = row;
            this.col = col;
            this.widths = [];
            this.heights = [];
    ...
    * 说明：右键菜单信息
    ...
    function CellType(hwReport) {
        ...
        */
        this.parse = function (celltypejson, enattr) {
            ...
            this.set("seperator", { semicolon: ";", comma: ",", number: "、" }[celltypejson[getAttrName("separator", enattr)]]);
        ...
        this.init = function (grid) {
            if (this.getType() == "vmdcombo") {
                ...
                if (rptStore && rptStore.dhtmlxDatastore) {
                    ...
                    if (!this.bindsource.multicolumns) {
    ...
    function RptSection(report) {
        ...
        * data 后台请求返回数据
        */
        this.compareGroup = function (data) {
        ...
        this.merge = function (n, firstGroup, lastGroup, ignore, array) {
            ...
            if (n == array.length) {//所有含分组函数的单元格已经比较完毕
                ...
                if (firstGroup[n].data == lastGroup[n].data) {
                    //改变上一组的rowspan                  
    ...
    */
    function RptStore(report, type) {
        var that = this;
        this.type = type == "submit" ? "submit" : "query"; //数据集是查询数据集还是入库数据集
        ...
        /*
        * filed字段名 index获取第几条记录的值 未定义时获取全部
        */
        this.getValue = function (field, index) {
        ...
        */
        ...
        this.update = function (success, error) {
            ...
            if (that._lastParams && isEqual(that._lastParams, params)) {
                if (success) {
                    success.apply(that, []);
                }
                return;
            ...
            hwDas.get(urlConfig, headers, params,
                function (result) {
                    ...
                    if (that.hwReport) {
                        var bindDatastore = that.hwReport.bindDatastore || that.hwReport.bindExtraDatastore;
                        if (["simple_grid", "free_grid"].indexOf(that.hwReport.showMode) != -1 && bindDatastore.name == that.name && !that.hwReport.totalcount) {
                            that.hwReport.totalcount = result.data[0].total;
                        }
                    }
                    else if (that.contexts.length > 0) {
                        for (var n = 0; n < that.contexts.length; n++){
                            that.contexts[n].totalcount = result.data[0].total;
                        }
                    }
            ...
            for (var i = 0; i < this.relativeCells.length; i++) {
                ...
                if (this.hwReport.checkParamType(expvalue) == "ds") {
                    ...
                    if (oCell.isTitle) {
                        ...
                        if (this.fixedColCount && oCell.rptRow.rowIndex < this.fixedColCount) {
                        ...
                        for (var j = 0; j < tdcells.length; j++) {
                            if (tdcells[j]._attrs && tdcells[j]._attrs.sid == oCell.id) {
                                tdcells[j].firstChild.innerHTML = this.hwReport.getValue(expvalue, c_index);
                                c_index++;
                            }
                        }
                    }
        ...
        this.addItem = function (data, index) {
            ...
            if (this.hwReport) {
                if (["simple_grid", "free_grid"].indexOf(this.hwReport.showMode) != -1 && this.hwReport.bindDatastore.name == this.name && !this.hwReport.totalcount) {
                    this.hwReport.totalcount = this.dhtmlxDatastore.dataCount();
                }
            }
            else if (this.contexts.length > 0) {
                for (var n = 0; n < this.contexts.length; n++) {
                    this.contexts[n].totalcount = this.dhtmlxDatastore.dataCount();
                }
            }
        }
        ...
        */
        this.fillDataByAdd = function (rId, rowsNum) {
            ...
            if (defined(this.baseIndex)) {
                var baseRule = bindRules[this.baseIndex];
                var baseOCell = hwReport.getOriginCellByLetter(baseRule.cellid);
                var baseCell;
                var rowIndex = grid.getRowIndex(rId);

                grid.forEachCellsB(function (cellObj, rInd, cInd) {
                    if (cellObj.cell._attrs.sid == baseOCell.id) {
                        baseCell = cellObj;
                    }
                ...
                function (result) {
                    ...
                    if (result.isSucceed) {
                        ...
                        if (that.hwReport && that.hwReport.grid.changedUploadCells) {
                            ...
                            for (var n = 0; n < that.contexts.length; n++) {
                                for (var cellid in that.contexts[n].grid.changedUploadCells) {
                                    var uploadCell = hat.contexts[n].grid.changedUploadCells[cellid];
                                    var uploader = uploadCell.cell._uploader;
                                    for (var fileId in uploader._files) {
                                        uploader._files[fileId].server = true;
                                    }
    ...
    * 报表对象
    */
    function HwReport(vmdreport, panelEl, container, width, height, isSubReport, gridAutoWidth, gridAutoHeight, subReportType) {
        ...
        this.showMode = "";//"free_grid"自由格式 "simple_grid"简单网格
        ...
        //if (parent.xds != undefined && !isSubReport && !window.__hasLoadedCss) {
        //    window.__hasLoadedCss = true;
        //    vmd.core.loadCss(vmd.virtualPath + '/css/common.css?r=' + Math.random());
        //    vmd.core.loadCss(vmd.virtualPath + '/lib/dhtmlx/skins/material/dhtmlx.css?r=' + Math.random());
        //    vmd.core.loadCss(vmd.virtualPath + '/css/shortcut.css?r=' + Math.random());
        //}
        ...
        this.loadJS = function (callback) {
            ...
            loadJSChains.push(reportUrl + "/hwreport_server.js");
        ...
        this.regenAutoWidths = function (colWidths, width) {
        ...
        this.parse = function (json) {
            ...
            for (var key in json.celltypes) {
                ...
                var celltype = new CellType(this);
        ...
        this.getQueryStoreByName = function (name) {
        ...
        * 设置标题浮动居中
        */
        this.setTitleFloat = function (bool) {
            if (this.isSubReport) {
                return;
            }
            ...
            if(this.winWidth){
                ...
                t_floatTitleContainer.style.width = Math.min(getElementWidth(this.panelEl), this.gridContainer.clientWidth) + "px";
            }
            var tr = document.createElement("tr");
            ...
            titlediv.innerHTML = replaceToHtmltag(titleText, true/*isEscape*/);
        ...
        */
        this.analysis = function () {
            ...
            if (that.expandType != "v" && !that.bindExtraDatastore) {
                this.loadMode = "";
            }
            ...
            this.storageDataStores.each(function (key, _store, index) {
                ...
                for (var i = 0; i < bindRules.length; i++) {
                    ...
                    if ((that.getExpandParent(oCell, "hparent") && that.getExpandParent(oCell, "hparent").dsFunc == "select")
                        || (that.getExpandParent(oCell, "vparent") && that.getExpandParent(oCell, "vparent").dsFunc == "select")) {
                        _store.baseIndex = i;
        ...
        //判断参数类型
        this.checkParamType = function (param) {
            //未定义
            if (!defined(param)) {
                return "undefined";
            }
        ...
        * 根据id获取原始单元格对象
        */
        this.getOriginCellById = function (id) {
            if (!/^\d+_\d+$/gi.test(id)) {
                return null;
            }
        ...
        * 根据字母数字格式获取原始单元格对象
        */
        this.getOriginCellByLetter = function (letter) {
        ...
        * 获取设置内联样式后的行高
        */
        this.getRowCSSHeight = function (cells) {
        ...
        this.setLoadMode = function (mode) {
            this.loadMode = mode;
        }
        ...
        */
        this.findMergingCell = function (rowIndex, cellIndex, rowInc, colInc) {
        ...
        * 补齐单元格
        */
        this.getPatchCell = function (rInd, cInd) {
            var that = this;
        ...
        this.clearGrid = function (header) {
            for (var i = 0; i < this.sections.length; i++) {
                var section = this.sections[i];
        ...
        * 根据行列索引获取表头dom元素
        */
        this.getHeaderCellDom = function (rInd, cInd) {
            var _grid = this.grid;
        ...
        */
        this.getParentRelation = function (groupParent1, groupParent2) {
        ...
        */
        this.getGridGroupArea = function (contextCell, paramOCell) {
        ...
        * 获取单元格对象
        */
        this.getCells = function (param, contextCell) {
            var that = this;
            contextCell = contextCell || this.grid.editor;
            ...
            if (relationH == 1) {
                for (var i = 0; i < paramOCell.childs.length; i++) {
                    ...
                    var contextRowIndex = that.grid.getRowIndex(contextCell.cell.parentNode.idd);
                    var paramRowIndex = that.grid.getRowIndex(paramCellObj.cell.parentNode.idd);
                    if (paramRowIndex >= contextRowIndex && paramRowIndex < (contextRowIndex + contextCell.cell.rowSpan)) {
                        cells.push(paramCellObj);
                    }
                }
                return cells;
        ...
        * 获取单元格的值
        */
        this.getCellValue = function (param, contextCell) {
            var cells = this.getCells(param, contextCell);
            return cells && cells.map(function (c) { return c.getValue(); }).join(",");
        }

        this.insertRules = function () {
            var that = this;
            if (!that.inlineStyle) {
                return;
            }

            //if (!this.selecteStates) {
            //    insertRule("#" + that.reportId + " table.obj.row20px tr.rowselected td", "background-color: #FFFFFF;");
            //}
        ...
        this.convertHeaders = function (originHeaders) {
            var rowSpanFlags = [];
            this.headers = [];
        ...
        this.convertRow = function (row, rowSpanFlags) {
            var values = [];
            var styles = [];
            var _class = [];
            var _attrs = [];
            ...
            for (var i = 0; i < row.length; i += colspan) {
                ...
                if (!rowSpanFlags[i] || rowSpanFlags[i] == 1) {
                    ...
                    if (split_c) {
                        ...
                        values[i] = replaceToHtmltag(originCell.isTitle ? "" : split_c.data,true /*isEscape*/) || "&nbsp;";
                        ...
                        values[i] = replaceToHtmltag(originCell.isTitle ? "" : c.data, true/*isEscape*/) || "&nbsp;";
                    }
                    if (originCell) {
        ...
        */
        ...
        //页面加载完成后填充入库数据集
        this.fillStorageOnRendered = function () {
            var that = this;
            this.storageDataStores.each(function (key, _store, index) {
        ...
        */
        this.getCellsDom = function () {
            ...
            //存放表头单元格
            var headerRowsNum = this.fixedRowCount - (this.floatTitleContainer ? 1 : 0);
            var startIndex;
            var temp_rspan = [];
            for (var i = 0; i < headerRowsNum + this.grid.getRowsNum() ; i++) {
                ...
                if (i < headerRowsNum) {
                    ...
                    if (this.loadMode == "smart" && this.showMode == "simple_grid") {
        ...
        this.creatPrintWindow = function () {
            var that = this;
            if (this.dhxPrintSetWindow) {
                return this.dhxPrintSetWindow;
            }
            this.dhxWindow = this.dhxWindow || new dhtmlXWindows();
            this.dhxPrintSetWindow = this.dhxPrintSetWindow || this.dhxWindow.createWindow("dhxPrintSetWindow", 0, 0, 800, 600);
            ...
            this.dhxPrintSetForm = this.dhxPrintSetForm || this.dhxPrintSetWindow.attachForm([
                
                {
                    type: "block", width: 345, blockOffset: 20, offsetTop: 10, list: [
                       {
                           type: "combo", position: "label-left", name: "format", offsetLeft: 0, label: "纸张大小：", labelWidth: 100, inputHeight: 50, inputWidth: 190,
                           options: [
                ...
                },
                {
                    type: "block", width: 345, blockOffset: 20, offsetTop: 10, list: [
                       {
                           type: "combo", position: "label-left", name: "format", offsetLeft: 0, label: "纸张方向：", labelWidth: 100, inputHeight: 50, inputWidth: 190,
                           options: [
                               { value: "image/png", text: "PNG", selected: true },
                               { value: "image/jpeg", text: "JPG" },
                               { value: "application/pdf", text: "PDF" },
                ...
                {
                    type: "block", width: 345, blockOffset: 20, offsetTop: 10, list: [
                       { type: "input", name: "height", labelWidth: 100, labelHeight: 18, width: 140, offsetLeft: 0, offsetTop: 10, inputHeight: 20, position: "label-left", label: "打印机分辨率：", value: "90" },
                       { type: "newcolumn" },
                       { type: "label", label: "点/英寸" }
                    ]
                },
                {
                    type: "block", width: 330, blockOffset: 5, offsetTop: 20, list: [

                       {
                           type: "fieldset", label: unescape("边距(mm)"), width: 280, offsetLeft: 10, offsetTop: 2, list: [
                              {
                                  type: "block", width: 260, blockOffset: 0, list: [
                                     { type: "input", name: "width", labelWidth: 25, labelHeight: 18, width: 85, offsetLeft: 0, offsetTop: 5, position: "label-left", label: "上:", value: "0" },
                                     { type: "newcolumn" },
                                     { type: "input", name: "width", labelWidth: 25, labelHeight: 18, width: 85, offsetLeft: 10, offsetTop: 5, position: "label-left", label: "下:", value: "0" }
                                  ]
                              },
                              {
                                  type: "block", width: 260, blockOffset: 0, list: [
                                     { type: "input", name: "height", labelWidth: 25, labelHeight: 18, width: 85, offsetLeft: 0, offsetTop: 5, position: "label-left", label: "左:", value: "0" },
                                     { type: "newcolumn" },
                                     { type: "input", name: "height", labelWidth: 25, labelHeight: 18, width: 85, offsetLeft: 10, offsetTop: 5, position: "label-left", label: "右:", value: "0" }
                                  ]
                              }
                           ]
                       }
                ...
                {
                    type: "block", offsetLeft: 80, list: [
                       { type: "button", width: 60, name: "confirm", value: unescape("%u786E%u5B9A"), offsetTop: 10, offsetLeft: 10 },
                       { type: "newcolumn" },
                       { type: "button", width: 60, name: "cancel", value: unescape("%u53D6%u6D88"), offsetTop: 10, offsetLeft: 10 }
                    ]
                }
            ]);
    ...
    HwReport.prototype = {
        initContainer: function () {
            ...
            var regExp = new RegExp(this.showMode, "gi");
            if (this.showMode && !regExp.test(this.rptContainer.className)) {
                this.rptContainer.className += " " + this.showMode;
            }
        ...
        */
        ...
        attachEvents: function () {
            ...
            this.attachEvent("onApproval", function (grid, rId, cInd, result, params) {
                ...
                if (oCell.eventId) {
                    var event = that.events.get(oCell.eventId);
                    that.vmdreport.fireEvent(event && event.approval, that, that.grid, cell, rId, cInd, result, params || {});
                }
            });
            
            this.attachEvent("onStoreSuccess", function (store) {
            ...
            this.attachEvent("onStoreAllSuccess", function () {
                    ...
                    if (that.loadMode == "paging" && !that._laypage) {
                        ...
                        that._laypage = laypage.render({
                            ...
                            jump: function (obj) {
                                ...
                                if (that.getPageSize() != pageSize || that.startIndex != startIndex) {
                                    that.pageSize = pageSize;
                                    that.startIndex = startIndex;
                                    that.bindDatastore._lastParams = null;
                                    that.bindDatastore.cache = {};
                                    that.bindDatastore.setPaging((obj.curr - 1) * obj.limit, obj.limit);
                                    that.bindDatastore.update();
                                    that.resetCounter();
                                }
                            }
                        });
                ...
                else if (this.showMode == "free_grid") {
                    that.setSize();
                    that.setPosition();
            ...
            this.attachEvent("onScrollToBottom", function () {
                ...
                if (that.showMode == "" && that.loadMode == "smart" && (that.rptType == "group" || that.rptType == "select")) {
        ...
        attachGridEvent: function () {
            ...
            this.grid.attachEvent("onEditCell", function doOnEditCell(stage, rId, cInd, newValue, oldValue) {
                ...
                if (stage == 2 && !isEqual(newValue, oldValue)) {
                    ...
                    //如果报表中绑定了主数据（如网格或自由格式），单元格值发生变化后修改数据集中的数据
                    if (that.showMode != "simple_grid") {
                        var _field = oCell.dsField && oCell.dsField[0];
                        var _dsName = oCell.dsName && oCell.dsName[0];
                        var bindStore = that.bindExtraDatastore || that.bindDatastore;
        ...
        setSize: function (w, h) {
            ...
            this.gridContainerW = this.gridContainerW = containerW;
            ...
            if (this.patchLastColIfLessWinW) {
                ...
                if (this.grid && (this.isSubReport && this.subReportType == "embed")) {
                    isVScroll = this.grid.obj.offsetHeight > containerH;
                }
                if (isVScroll) {
            ...
            if (this.grid) {
                ...
                if (!dhx4.isIE) {
                    isVScroll = this.isSubReport ? false : (this.grid.obj.offsetHeight > (h ? (h - this.grid.hdr.offsetHeight) : this.grid.objBox.offsetHeight));
                    isHScroll = this.isSubReport ? false : ((this.grid.obj.offsetWidth > this.grid.objBox.offsetWidth) || (this.grid.objBox.style.overflowX == "scroll") || (gridWidth > containerW));
                }
                else {
                    isVScroll = this.isSubReport ? false : (this.grid.obj.offsetHeight > (h ? (h - this.grid.hdr.offsetHeight) : this.grid.objBox.offsetHeight));
                    isHScroll = this.isSubReport ? false : ((this.grid.obj.offsetWidth > containerW) || (this.grid.objBox.style.overflowX == "scroll") || (gridWidth > containerW));
                }
            }
            ...
            if (this.grid) {
                ...
                if (this.floatTitleContainer) {
                    if(this.winWidth){
                        ...
                        this.floatTitleContainer.style.width = Math.min(getElementWidth(this.panelEl), this.gridContainer.clientWidth) + "px";
                    }
                }
            }
        },
        ...
        */
        updateDatastore: function () {
            var that = this;
            if (this.queryDatastores.size() == 0) {
            ...
            function checkAllComplete() {
                var allComplete = true;
                that.queryDatastores.each(function (key, _store, index) {
        ...
        //全局校验
        gloabalValidate: function () {
            var validateResult = true;
            this.grid.updateFormulaVariable();
            for (var i = 0; i < this.ValidateListJSON.length; i++) {
                var validateItem = this.ValidateListJSON[i];
                if (validateItem.ignore) {
                    continue;
                }
                var hwExp = new HWJE(validateItem._exp, this.formulaVariablesMap);
                var resText = hwExp.getExpVal();
                if (!resText) {
                    showAlert("校验失败！", validateItem.toolTip || '校验失败！', "error");
        ...
        */
        getValue: function (exp, contextCell) {
            ...
            return this.rptExpress.calculate(exp, 1, contextCell);
        },
        ...
        */
        ...
        //将合并单元格添加到下一行中
        appentMergeCellToNextRow: function (row, mergedCell) {
                ...
                for (var startColIndex = this.grid._cCount - 1; startColIndex >= 0; startColIndex -= colspan) {
                    ...
                    if (mergedCell.cell.rowSpan > 1) {
                        ...
                        if (mergedCell.cell.parentNode.idd == deleteId) {
                            that.appentMergeCellToNextRow(row, mergedCell);
                        }
                        else {
                            mergedCell.cell.rowSpan -= 1;
                        }
        ...
        */
        ...
        //在外部绑定dhtmlx的数据集
        setBindExtraDatastore: function (rptDataStore) {
            var that = this;
            this.bindExtraDatastore = rptDataStore;
            rptDataStore.contexts.push(this);
        },
        ...
        */
        print: function (options) {
            ...
            this.rptPageSet.setOptions(options);
            var doms = this.getCellsDom();
            var widths = this.grid.getColsWidth();
            if (this.patchLastColIfLessWinW) {
                widths.pop();
            }
            var pageDom = this.rptPageSet.genPages(widths, this.grid.getRowsHeight(), doms.title, doms.header, doms.data);
            myMask.hide();
            $(pageDom).print();
        },
        ...
        exportExcel: function (options) {
            ...
            for (var i = 0; i < all_datas.length; i++){
                ...
                for (var j = 0; j < cCount; j += colspan_ind) {
                    ...
                    var oCell = this.getOriginCellById(td._attrs.sid);
                    if (!oCell) {
                        row_json.push({});
                        colspan_ind = 1;
                        continue;
                    ...
                    if (oCell.backgroundColor) {
                        attrs.backgroundColor = oCell.backgroundColor;
                    }
