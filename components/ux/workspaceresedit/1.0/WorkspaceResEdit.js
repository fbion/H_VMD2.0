Ext.define("vmd.ux.WorkspaceResEdit" ,{
	extend:"Ext.Panel",
	...
	height:522,
	layout:"auto",
	afterrender:"WorkspaceResEdit_afterrender",
	listeners:{
		vmdafterrender:function(){
	this.WorkspaceResEdit_afterrender(this)
}
	},
                        ...
                        if(null != result.data[0].datas && undefined != result.data[0].datas) {
                            if(result.data[0].datas.length > 0) {
                                ...
                                for(var i = 0; i < result.data[0].datas.length; i++) {
                                    ...
                                    if(1 == resouType || 2 == resouType) {
                                        loadResourceCategory(nodeService, objInfoId, result.data[0].datas[i].id, resouType);
                                    }
                                }
                            } else {
                                clearTree();
...
function WorkspaceResEdit_afterrender(sender) {
    div.hide()
}
...
function WorkspaceResEdit_beforerender(sender){

}
			this.WorkspaceResEdit_afterrender=WorkspaceResEdit_afterrender;
		this.WorkspaceResEdit_beforerender=WorkspaceResEdit_beforerender;
		this.items=[
			{
				xtype:"vmd.div",
				id:"div",
...
addServeInServe(id, name, addr, remark, resType)
	...
	Ext.util.CSS.removeStyleSheet("vmd.ux.WorkspaceResEdit");
		this.uxCss&&Ext.util.CSS.createStyleSheet(vmd.replaceResVars(this.uxCss), "vmd.ux.WorkspaceResEdit");
	}
})