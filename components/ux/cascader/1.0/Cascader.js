Ext.define("vmd.ux.Cascader", {
    ...
    baseCls: "",
    cls: "ys",
    beforerender: "Cascader_beforerender",
    listeners: {
        beforerender: function() {
            this.Cascader_beforerender(this)
        }
    },
    detailWidth: 200,
    comboWidth: 300,
    ...
    initComponent: function() {
        ...
        function Combo(option) {
            ...
            this.expandTrigger = option.expandTrigger || '';
        }
        ...
        /*------------------------------confirmTabs为false---------------------------------------------*/
        function getAllData(root, store, valueField, displayField, parentField) {
            if (!store) return;
            var datas = store.getJson();
            var TabData1 = [];
            // for (var i = 0, len = data.length; i < len; i++) {
            // }
            datas.forEach(function(data) {
                if (data.get(parentField) == root) {
                    TabData1.push(data);
                }
            })
            ...
            MyTabs.add(panel);
            return getFistTab(TabData1)
        }

        function getFistTab(datas) {
            if (MyTabs.getActiveTab() == firstTab) {
                firstTab.removeAll();
            }
            // if (!provinceData) {
            //     return;
            // }
            // var datas = datasprovinceData.data[0].datas;
            for (var i = 0, len = datas.length; i < len; i++) {
                var item_width, label_cls;
                if (datas[i].name.length <= 5) {
                    item_width = 90;
        ...
        /*------------------------------confirmTabs为true---------------------------------------------*/
        function getTabsData(stores) {}

        function combo_beforerender(sender) {}

        function Cascader_beforerender(sender) {
            var inistcombo = new Combo({
                id: '',
            });
            Ext.apply(combo, inistcombo)
        }
        this.Cascader_beforerender = Cascader_beforerender;
        this.items = [{
                xtype: "vmd.combo",
                id: "combo",
        ...
        Ext.util.CSS.removeStyleSheet("vmd.ux.Cascader");
        this.uxCss && Ext.util.CSS.createStyleSheet(vmd.replaceResVars(this.uxCss), "vmd.ux.Cascader");
    }
})