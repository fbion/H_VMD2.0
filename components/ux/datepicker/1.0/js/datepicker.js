...
$(function () {
  ...
  $.extend(Day.prototype, {
    ...
    eventSingle: function () {
      ...
      function clickDate(_this,$target){
		...
		//增加oldval
		//_this.picker.oldval=_this.picker.$input.val();
        if (!val) {
      ...
      function setValue(activeNum, input, inputDay) {
		...
		//统一处理select，change事件
		this.picker.config.select(inputVal);
		var oldval=this.picker.datePickerObject.initInputVal;
		if(oldval!=inputVal &&!this.picker.hasTime) this.picker.config.change(inputVal,oldval);
      }
    },
    eventRange: function () {
      // 点击选择日期
      ...
      function clickDateRange(_this,$target){
		...
		var _oldval={
			start:_this.picker.$inputBegin.val(),
			end:_this.picker.$inputEnd.val()
		}
        if (!_this.current) {
          $target.addClass('current');
		  ...
		  //绑定事件
		   _this.picker.config.select([newval.start,newval.end]);
		   _this.picker.config.change([newval.start,newval.end],[_oldval.start,_oldval.end]);
  ...
  $.extend(Time.prototype, {
    ...
    event: function () {
	  ...
	  //增加时分秒点击事件处理
	  this.picker.$container.find('.c-datepicker-scrollbar__wrap').mousedown(function(e){
      ...
      if ($(panel).css('display') === 'block') {
        ...
        if (_this.config.isRange && (!_this.$inputBegin.val() && !_this.$inputEnd.val())) {
          $(panel).find('td.available').removeClass('current in-range');
			...
			newval=_this.$input.val();
			oldval=_this.datePickerObject.initInputVal;
			if(oldval!=newval) _this.config.change(newval,oldval);
		}
        if (_this.hasTime) {
  ...
  $.extend(SingleDatePicker.prototype, {
    ...
    event: function () {
      ...
      this.$container.on('click', '.c-datepicker-picker__btn-now', function () {
		...
		//统一处理select，change事件
		var inputVal=_this.$input.val();
		_this.config.select(inputVal);
		var oldval=_this.datePickerObject.initInputVal;
		if(oldval!=inputVal) _this.config.change(inputVal,oldval);
      });
      ...
      this.$container.on('click', '.c-datepicker-picker__shortcut', function () {
		...
		var oldval=_this.$input.val();
        setValue(_this, result);
		var newval=_this.$input.val();
		_this.config.select(newval);
		_this.config.change(newval,oldval);
      ...
      this.$container.on('click', '.c-datepicker-picker__link-btn.confirm', function () {
        ...
        if (!_this.$input.val()) {
          setValue(_this, moment().format(_this.config.format));
			...
			_this.config.select(_this.$input.val());
			var oldval=_this.datePickerObject.initInputVal;
			if(oldval!=_this.$input.val())
			 _this.config.change(_this.$input.val(),oldval);	
		}
  ...
  $.extend(RangeDatePicker.prototype, {
    ...
    event: function () {
      ...
      this.$container.on('click', '.c-datepicker-picker__shortcut', function () {
		...
		//有效期处理
		if($.trim(days[1])=='36500') end='9999-12-31';
        if (_this.hasTime) {
    ...
    show: function () {
      ...
      var cl=parseInt(this.$container.css('left'));
      var ct=parseInt(this.$container.css('top'));
      var cw=this.$container.width();
      var ch=this.$container.height();
      if((cl+cw)>bw)   this.$container.css('left',bw-cw);
      if(ct+ch>bh)  this.$container.css('top',bh-ct);
      this.$container.show();
    },
    clear: function () {
      this.$inputBegin.val('');
  ...
  function setContainerPos(_this) {
	...
	var datapickerH=_this.pickerObject.$container.outerHeight();
	if(bodyH-offset.top - height<datapickerH){
		if(offset.top >datapickerH){
			_this.pickerObject.$container.css({
			  top: offset.top-datapickerH-5,
			  left: offset.left
			});
		}else{
			_this.pickerObject.$container.css({
		  top: bodyH-datapickerH-5,
		  left: offset.left
		});
		}
	}else
