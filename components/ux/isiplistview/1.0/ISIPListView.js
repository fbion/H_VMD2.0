Ext.define("vmd.ux.ISIPListView" ,{
	extend:"Ext.Panel",
	...
	height:472,
	layout:"border",
	afterrender:"ISIPListView_afterrender",
	listeners:{
		vmdafterrender:function(){
	this.ISIPListView_afterrender(this)
}
	},
...
function ISIPListView_afterrender(sender) {
...
function node_del(DataView, index, node, e) {
    page.fireEvent('moduleDelete', DataView, index, node, e);
}

function node_edit(DataView, index, node, e) {
    page.fireEvent('moduleEdit', DataView, index, node, e);
}

function node_folder(DataView) {
    page.fireEvent('addFolder', DataView);
}

function node_module(DataView) {
    page.fireEvent('addModule', DataView);
}
...
function hwDataView_afterrender(sender) {
    ...
    this.on("mouseenter", function(DataView, index, node, e) {
        if(node.attributes.ptype && node.attributes.ptype.nodeValue == "add") {
            ...
            nodeEl.append('<div class="nodetool" style="display: inherit;width:25px;height:50px;margin-top"><span class="button folder"  title="分类" onfocus="this.blur();"></span><span class="button module"  title="模块"></div>');
            node._tool = nodeEl[0].lastChild;
            node._tool.folder = nodeEl[0].lastChild.firstChild;
            node._tool.module = nodeEl[0].lastChild.lastChild;
            vmd(node._tool.folder).bind('click', function(){node_folder(DataView)});
            vmd(node._tool.module).bind('click', function(){node_module(DataView)});
        } else {
...
function hwDataView_click(sender, index, node, e) {
    ...
    if(allcount == index) {
        page.fireEvent("addModule", sender);
    } else {
...
function MyGrid_afterrender(sender) {
    ...
    nodeEl.append('<div class="nodetool" style="width: 100%-10;height: 30px;margin-top;border: 1px blue dashed;"><div class="filename" style="margin-left: 50%;"><span class="ico x-item-file x-add-small" onfocus="this.blur();"></span><span title="新建" style="line-height: 30px;">新建</span></div></div>');
    ...
    vmd(sender._tool.iico).bind('click', function(e){listAddClick(listview,e)});
    vmd(sender._tool.itext).bind('click', function(e){listAddClick(listview,e)});
...
function hwMenuItem_click(sender,e){
    ...
    page.fireEvent('addFolder', this.ownerCt.listview);
}


function hwMenuItem1_click(sender,e){
    page.fireEvent('addModule', this.ownerCt.listview);
}
			this.ISIPListView_afterrender=ISIPListView_afterrender;
		this.items=[
			{
				xtype:"vmd.div",
				id:"div",
					...
					},
					{
						...
						columns:[
							...
							},
							{
								...
								dataIndex:"name",
								width:200,
								tpl:"<div class=\"filename\">    <div class=\"ico\" filetype=\"folder\">        <i class=\"x-item-file x-{type} small\">        </i>    </div>    <span title=\"{name}\">{name}</span></div>",
								menuDisabled:true
							},
							{
								xtype:"templatecolumn",
...
for(var i = 0; i < hwDataView.store.data.items.length; i++) {
    if(selIds.indexOf(hwDataView.store.data.items[i].data.id) >= 0) {
        if(!MyGrid.hidden) {
...
if(hidden) {
	...
	Ext.util.CSS.removeStyleSheet("vmd.ux.ISIPListView");
		this.uxCss&&Ext.util.CSS.createStyleSheet(vmd.replaceResVars(this.uxCss), "vmd.ux.ISIPListView");
	}
})