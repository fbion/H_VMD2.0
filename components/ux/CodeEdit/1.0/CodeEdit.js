Ext.define("vmd.ux.CodeEdit", {
    ...
    uxrequirejs: "[\"lib/ace/ace.js?ver=vmd2.0.4.190810\",\"lib/ace/mode-base.js?ver=vmd2.0.4.190810\",\"lib/ace/theme-xcode.js?ver=vmd2.0.4.190810\",\"lib/ace/ext-language_tools.js?ver=vmd2.0.4.190810\"]",
    initComponent: function() {
        ...
        try {
            ...
            function button_codeedit_click(sender, e) {
                var reInfo = {
                    type: "0",
                    value: expressionDisplay.getValue(),
                    remark: coderemark.getValue()
                }
                var exp = JSON.stringify(reInfo);
                if (expressionDisplay.getValue().trim() == "")
                    parent.simpleGridCodeEdit.setBack("", true);
                else
                    parent.simpleGridCodeEdit.setBack(exp, true);
                parent.simpleGridCodeEdit.close()
            }

            function button1_codeedit_click(sender, e) {
                parent.simpleGridCodeEdit.close()
            }
            var lastEditCode = ""
            var lastEditVisual = ""
            ...
            function _setValue(value, remark, type) {
                ...
                if (subType == "0") {
                    ...
                    lastEditVisual = expressionDisplay.getValue()
                }
                subType = type;
                if (type == "0") {
                    hwDiv_codeedit.show()
                    hwDiv2_codeedit.hide()
                    expressionDisplay.setValue(lastEditCode || value);
                    //expressionDisplay.language = "javascript"
                    expressionDisplay.getSession().setMode("ace/mode/javascript")
                    hwDiv1_codeedit.add(expressionDisplay)
                    hwDiv1_codeedit.doLayout()
                    hwDiv_codeedit.doLayout()
                    coderemark.setValue(remark)
                } else {
                    hwDiv2_codeedit.show()
                    hwDiv_codeedit.hide()
                    expressionDisplay.setValue(lastEditVisual || value)
                    expressionDisplay.getSession().setMode("ace/mode/html")
                    //expressionDisplay.language = "html"
                    hwDiv.add(expressionDisplay)
                    hwDiv.doLayout()
                    hwDiv2_codeedit.doLayout()
                }
            }

            function code_edit_afterrender(sender) {
            ...
            // {
            //     //expressionDisplay.setValue(value)
            // }
            ...
            function determine_click(sender, e) {
                ...
                if (exp.trim() == "")
                    ...
                    parent.simpleGridCodeEdit.setBack(JSON.stringify(reInfo), true);
                parent.simpleGridCodeEdit.close()
            }
            ...
            function expressionDisplay_afterrender(sender) {
                sender.toolbar.hide() //隐藏工具栏
                sender.setOption("wrap", "free") //自动换行
            }
        } catch (ex) {
                ...
                items: [{
                    ...
                    },
                    {
                        ...
                        items: [{
                            ...
                            x: 10,
                            y: 30,
                            afterrender: "expressionDisplay_afterrender",
                            listeners: {
                                vmdafterrender: expressionDisplay_afterrender
                            }
                        }]
                    }
                ]
        ...
        eval(me.defineVars);
        resetCmpScope();
        this.setValue = function(value, remark, type) {
            //直接填写方法内容
            _setValue(value, remark, type)
        }
        Ext.util.CSS.removeStyleSheet("vmd.ux.CodeEdit");
        this.uxCss && Ext.util.CSS.createStyleSheet(vmd.replaceResVars(this.uxCss), "vmd.ux.CodeEdit");
    }
})