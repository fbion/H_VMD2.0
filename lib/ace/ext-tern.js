...
ace.define("ace/tern/tern_server",["require","exports","module","ace/range","ace/lib/dom"], function (require, exports, module) {
        ...
        if (this.options.useWorker) {
            ...
            if (this.options.defs && this.options.defs.length > 0) {
                ...
                for (var i = 0; i < this.options.defs.length; i++) {
                    def = window['def_' + this.options.defs[i]];
    ...
    function getFragmentAround(data, start, end) {
        ...
        var max = Math.min(editor.session.getLength(), end.line + 20);
    ...
    function getCompletions(ts, editor, session, pos, prefix, callback) {
        ...
        var controlPrefix = "__control__"
        var getCompletionItem = function (item) {
            var type = item.type,
                score;
            if (type) {
                if (type.indexOf("fn") != -1) {
                    item.meta = "function";
                }
                else if (type.indexOf(controlPrefix) == 0 && item.origin == "control" && type.indexOf(".") == -1) {
                    item.meta = "instance";
                }
                else if (item.origin == "[doc]") {
                    item.meta = "local";
                }
                else {
                    item.meta = "property";
        ...
        var filterCompletions = function (items) {
            var ret = [],
                check = function (item) {
                    if (item.origin == "control" && item.name.indexOf(controlPrefix) == 0) {
                        return false;
                    }
                    return true;
                }
        ...
        var clearMakeTooltipTimer = function () {
            if (ts.makeTooltipTimer) {
                clearTimeout(ts.makeTooltipTimer);
                ts.makeTooltipTimer = null;
            }
        }
        ...
        ts.request(editor, {
            ...
            function (error, data) {
                ...
                data.completions.sort(function (a, b) {
                    return a.name.toLowerCase() > b.name.toLowerCase() ? 1 : -1
                });

                data.completions = filterCompletions(data.completions);
                var ternCompletions = data.completions.map(function (item, index) {
                    var _item = getCompletionItem(item, index);
                    return {
                        iconClass: " " + (_item.guess ? cls + "guess" : typeToIcon(_item.type)),
                        doc: _item.doc,
                        type: _item.type,
                        caption: _item.name,
                        value: _item.name,
                        score: _item.score - index,
                        meta: _item.meta
                    };
                });
                    ...
                    try {
                        ts.aceTextCompletor.getCompletions(editor, session, pos, prefix, function (error, data) {
                            ...
                            for (var z = 0; z < textCompletions.length; z++) {
                                ...
                                if (!item.value || otherCompletionsContains(item.value)) {
                                    continue;
                                }
                ...
                otherCompletions = filterCompletions(otherCompletions);
                ...
                var tooltip = null;
                //修复
                editor.completer.popup&&editor.completer.popup.on('select', popupSelectionChanged);
                if (!bindPopupSelect()) {
                ...
                function popupSelectionChanged() {
                   // alert('')
                    ...
                    ts.makeTooltipTimer = setTimeout(function () {
                        var popup = editor.completer.popup,
                            data = popup ? popup.getData(popup.getRow()) : undefined,
                            node,
                            nodeRect;
                        ...
                        node = popup.renderer.getContainerElement();
                        nodeRect = node.getBoundingClientRect();
                        tooltip = makeTooltip(nodeRect.right + window.pageXOffset, nodeRect.top + window.pageYOffset, createInfoDataTip(data, true), editor);
    ...
    function createInfoDataTip(data, includeType, activeArg) {
        ...
        if (data.tipHtml) {
            tip.appendChild(elFromString(data.tipHtml));
            return tip;
        }
    ...
    function getCusorPosForTooltip(editor) {
        var ret,
            ...
            place = renderer.$cursorLayer.getPixelPosition(),
            wrapRect = editor.renderer.container.getBoundingClientRect();
        place.top += wrapRect.top + renderer.layerConfig.lineHeight;
        place.left += wrapRect.left + renderer.gutterWidth;

        place.top -= editor.renderer.scrollTop;
        place.left -= editor.renderer.scrollLeft;
        ret = {
            left: place.left,
            top: place.top
        };
    ...
    function updateArgHints(ts, editor) {
        ...
        function inner() {
            var currentPopup = editor.completer && editor.completer.popup;
            ...
            ts.request(editor, {
                ...
                if (data.origin == "control" || data.origin == "widget") {
                    ts.cachedArgHints.tipHtml = data.doc;
                }
    ...
    function showTooltipInWindow(tip, editor, args) {
        var wrap = {
            width: window.innerWidth,
            height: window.innerHeight
        },
        rect = tip.getBoundingClientRect(),
        isArgHints = !!args.isArgHints,
        popup,
        tipStyle = tip.style,
        popupRect,
        ret = {};
        if (rect.top + rect.height > wrap.height) {
            ret.top = isArgHints ?
                rect.top - rect.height - editor.renderer.layerConfig.lineHeight :
                wrap.height - rect.height;
            ret.top = Math.max(ret.top, 0);
        }
        if (rect.left + rect.width > wrap.width) {
            if (isArgHints) {
                ret.left = wrap.width - rect.width;
            }
            else {
                popup = editor.completer && editor.completer.popup;
                popupRect = popup ? popup.container.getBoundingClientRect() : { left: 0 };
                tipStyle.left = "auto";
                tipStyle.right = (wrap.width - popupRect.left) + "px";
            }
            ret.left = Math.max(ret.left, 0);
        }
        if (ret.hasOwnProperty("top")) {
            tipStyle.top = ret.top + "px";
        }
        if (ret.hasOwnProperty("left")) {
            tipStyle.left = ret.left + "px";
        }
    }
    function makeTooltip(x, y, content, editor, closeOnCusorActivity, fadeOutDuration) {
        ...
        showTooltipInWindow(node, editor, { isArgHints: content.isArgHints });
        delete content.isArgHints;
...
ace.define("ace/tern/tern",["require","exports","module","ace/config","ace/snippets","ace/autocomplete/text_completer","ace/autocomplete","ace/tern/tern_server","ace/editor"], function (require, exports, module) {
    ...
    var onAfterExec_Tern = function (e, commandManager) {
        if (e.editor.ternServer) {
            e.editor.ternServer.hideDoc();
        }
        ...
        if (e.command.name === "insertstring") {
            ...
            if (str == ".") {
                ...
                var pos = e.editor.getCursorPosition(),
                    line = e.editor.session.getLine(pos.row),
                    util = ace.require("ace/autocomplete/util"),
                    tok = e.editor.session.getTokenAt(pos.row, pos.column),
                    prefix = util.retrievePrecedingIdentifier(line, pos.column),
                    strIndex;
                if (prefix && prefix.length == 1 && tok.type !== "string") {
                    e.editor.execCommand("startAutocomplete");
                }
            }
        }
