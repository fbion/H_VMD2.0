...
(function () {
    ...
    window.onload = function () {
		 ...
		 $.get(bootPATH+'lib/ueditor/dialogs/config.json', function (data) {
            ...
            utils.options.serverUrl = editor.getActionUrl('config');
			 $.extend(editor.options,data)  
            initTabs();
            initButtons();
        }, 'text')
    ...
    UploadImage.prototype = {
        ...
        getInsertList: function () {
            var i, data, list = [],
                align = getAlign(),
                prefix =editor.getBasePath()|| editor.getOpt('imageUrlPrefix');
        ...
        getImageData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
               ...
               /* ajax.request(url, {
				...
				editor.getDirs&&editor.getDirs(editor.getOpt('imageManagerAllowFiles'),function(r){
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.width = 113;
                    img.setAttribute('src', editor.getBasePath()+ list[i].url + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    img.setAttribute('_src', editor.getBasePath()+ list[i].url);
