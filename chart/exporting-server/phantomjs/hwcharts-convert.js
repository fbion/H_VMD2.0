/**
 * @license HWcharts JS v3.0.1 (2012-11-02)
 *
 * (c) 20013-2014
 *
...
/*jslint white: true */
/*global window, require, phantom, console, $, document, Image, HWcharts, clearTimeout, clearInterval, options, cb, globalOptions, dataOptions, customCode */
...
(function () {
	...
	var config = {
			 ...
			 * Depending on purchased license change the HWcharts property to
			 * hwcharts.js or HWstock.js
			 */
			files: { 
				JQUERY: 'jquery.1.9.1.min.js',
				HWcharts: 'hwcharts.js',
				/* HWcharts: 'hwcharts.js',*/
				HWcharts_MORE: 'hwcharts-more.js',
				HWcharts_DATA: 'data.js',
				HWcharts_DRILLDOWN: 'drilldown.js',
				HWcharts_FUNNEL: 'funnel.js',
				HWcharts_HEATMAP: 'heatmap.js',
				HWcharts_3D: 'hwcharts-3d.js',
				HWcharts_NODATA: 'no-data-to-display.js',
				/*HWcharts_MAP: 'map.js',*/
				HWcharts_SOLID_GAUGE: 'solid-gauge.js',
	...
	mapCLArguments = function () {
		...
		if (system.args.length < 1) {
			console.log('Commandline Usage: hwcharts-convert.js -infile URL -outfile filename -scale 2.5 -width 300 -constr Chart -callback callback.js');
			console.log(', or run PhantomJS as server: hwcharts-convert.js -host 127.0.0.1 -port 1234');
		}
	...
	render = function (params, exitCallback) {
		...
		messages.optionsParsed = 'HWcharts.options.parsed';
		messages.callbackParsed = 'HWcharts.cb.parsed';
		...
		createChart = function (constr, input, globalOptionsArg, dataOptionsArg, customCodeArg, outputType, callback, messages) {
			...
			function loadScript(varStr, codeStr) {
				...
				if (window[varStr] !== undefined) {
					console.log('HWcharts.' + varStr + '.parsed');
				}
			}
			...
			function parseData(completeHandler, chartOptions, dataConfig) {
				try {
					...
					HWcharts.data(dataConfig, chartOptions);
				} catch (error) {
			...
			// disable animations
			HWcharts.SVGRenderer.prototype.Element.prototype.animate = HWcharts.SVGRenderer.prototype.Element.prototype.attr;
			HWcharts.setOptions({
				plotOptions: {
					series: {
						animation: false
			...
			// Load globalOptions
			if (globalOptions) {
				HWcharts.setOptions(globalOptions);
			}
			...
			if (dataOptions) {
				parseData(function completeHandler(opts) {
					...
					if (options.series) {
						HWcharts.each(options.series, function (series, i) {
							options.series[i] = HWcharts.merge(series, opts.series[i]);
						});
					}

					var mergedOptions = HWcharts.merge(opts, options);
					...
					chart = new HWcharts[constr](mergedOptions, cb);

				}, options, dataOptions);
			} else {
				chart = new HWcharts[constr](options, cb);
			}
			...
			*/
			...
			return {				
				html: $('div.hwcharts-container')[0].innerHTML,
		...
		if (params.length < 1) {
			...
			page.open('about:blank', function (status) {
				...
				if (input.substring(0, 4).toLowerCase() === "<svg" || input.substring(0, 5).toLowerCase() === "<?xml"
					...
					// We have a js file, let hwcharts create the chart first and grab the svg
