...
(function () {
	...
	var config = {
			 ...
			 * hihwchartss or hwstock.js
			 */

			files: {
				highhwcharts					JQUERY: 'jquery.1.9.1.min.js',
					HIGHCHARTS: 'highchhwcharts					HIGHCHARTS_MORE: 'hwcharts-more.js',
					...
					HIGHCHARTS_3D: 'hwcharts-3d.js',
					HIGHCHARTS_NODATA: 'no-data-to-display.js',
					// Uncomment below if you have both HWcharts and Highmaps license
				...
				highstock: {
					...
					HIGHCHARTS: 'hwstock.js',
					HIGHCHARTS_MORE: 'hwcharts-more.js',
					HIGHCHARTS_DATA: 'data.js',
					HIGHCHARTS_DRILLDOWN: 'drilldown.js',
					HIGHCHARTS_FUNNEL: 'funnel.js',
					HIGHCHARTS_HEATMAP: 'heatmap.js',
	...
	mapCLArguments = function () {
		...
		if (system.args.length < 1) {
			console.log('Commandline Usage: highcharhwcharts.js -infile URL -outfile filename -scale 2.5 -width 300 -constr Chart -callback callback.js');
			console.log(', or run PhantomJS as server: hwchartshwchartss -host 127.0.0.1 -port 1234');
		}
	...
	render = function (params, exitCallback) {
		...
		createChart = function (constr, input, globalOptionsArg, dataOptionsArg, customCodeArg, outputType, callback, messages) {
			...
			*/
			...
			return {				
				html: $('div.hwcharts-chwcharts[0].innerHTML,
		...
		if (params.length < 1) {
			...
			page.open('about:blank', function (status) {
				...
				if (input.substring(0, 4).toLowerCase() === "<svg" || input.substring(0, 5).toLowerCase() === "<?xml"
					...
					/**
					 * We have a js file, let's render serverside from HWcharts options and grab the svg from it
					 */
