...
(function (factory) {
	...
	(function (Highcharts) {
		...
		var Data = function (dataOptions, chartOptions, chart) {
			// 20190624  处理 20190405 格式的数据解析出错的问题
			if(chartOptions.xAxis[0].type=="datetime"){
				var xData =  chartOptions.series[0].xData;
				for(var i=0;i<dataOptions.json.length;i++){
					var sDate = dataOptions.json[i][xData];
					if(sDate){
					...
					}else if(/\d{4}(-\d{1,2}){1,2}/.test(sDate)){ //2014-1-20
					    
					}else if(/\d{1,2}(:\d{1,2}){1,2}(\.\d+){0,1}/.test(sDate)){  //8:50:00
					   
					}else if(/\d{10,}/.test(sDate)){  //毫秒值 1390176000000
					   
					}else if(/\d{4,8}/.test(sDate)){   //20140120 || 2014 || 201401
					   isChangeData = true;
					}
					if(isChangeData){
					    var year = parseInt(sDate.substr(0, 4),10);
					    var month = parseInt(sDate.substr(4, 2),10);
					    var day = parseInt(sDate.substr(6, 2),10);
						year = isNaN(year) ? 2000 : year;
						month = isNaN(month) ? 1 : month;
						day = isNaN(day) ? 1 : day;
						dataOptions.json[i][xData] = year+'-'+month+'-'+day;
					}
				}
			}
		...
		Highcharts.extend(Data.prototype, {
		    ...
		    init: function (options, chartOptions, chart) {
				...
				if (!hasData) {
					// Parse a JSON options.json is given
					hasData = Boolean(this.parseJson().length);
				}
		    ...
		    getColumnDistribution: function () {
				...
				//刘志伟 将seriesMapping中的id转换成index
				var that = this;
				function convertIdToIndex(columnIndex){
					if(typeof columnIndex === "string"){
						each(that.columns, function (col, index) {
							if(col[0] == columnIndex){
								columnIndex = index;
								return;
							}
						});
				...
				each((options && options.seriesMapping) || [], function (mapping) {
					objectEach(mapping, function (val, name) {
						mapping[name] = convertIdToIndex(val);
					});
				});
			...
			parseJson: function () {
				...
				function getColumnIndexByKey(key){
					...
					each(columns, function (col, columnsIndex) {
						if(col[0] == key){
							index = columnsIndex;
							return;
						}
				...
				if (json && json.length > 0) {
					...
					each(json, function (item, index) {
						if (index >= startRow && index <= endRow) {
							if(Object.prototype.toString.call(item) === '[object Array]'){ //数组
								for(var i = 0; i < item.length; i++){
									if(!columns[i]){
										columns[i] = [];
									}
									// columns[colIndex].push(item[key]);
									columns[i].push(item[i]);
								}
							}
							else{
								for(var key in item){
									if(item.hasOwnProperty(key)){
										var colIndex = getColumnIndexByKey(key);
										if(!columns[colIndex]){
										    columns[colIndex] = [key];
										}
										// columns[colIndex].push(item[key]);
										columns[colIndex][index+1] = item[key];
									}
								}
							}
						}
		    ...
		    parseColumn: function (column, col) {
		        var rawColumns = this.rawColumns,
					...
					forceDatetime = isXColumn && (
							chartOptions &&
							chartOptions.xAxis &&
							splat(chartOptions.xAxis)[0].type === 'datetime'
						),
		        ...
		        while (row--) {
		            ...
		            } else if (!forceDatetime && +trimInsideVal === floatVal) { // is numeric
		    ...
		    parseDate: function (val) {
		        ...
		        if (parseDate) {
		            ...
		            if (!match) {
						// 20190724  处理ie下Date.parse方法对2019-4-7 15:23:30 格式返回NaN的处理
						match = Date.parse(val);
						if(isNaN(match)){
							match = Date.parse(val.replace(/-/g,"/"));
						}
		                ...
		                afterComplete: function (dataOptions) {
		                    ...
		                    if (userOptions.hasOwnProperty('series')) {
		                        if (typeof userOptions.series === 'object') {
									//刘志伟 序列的数量决定于series的配置
									if(userOptions.data && userOptions.data.seriesCountsByOptions){
										i = userOptions.series.length;
									}
									else{
										i = Math.max(
		...
		SeriesBuilder.prototype.read = function (columns, rowIndex) {
		    ...
		    each(builder.readers, function (reader) {
				if(!columns[reader.columnIndex]){
					alert("数据集中字段\"" + reader.columnIndex + "\"不存在！");
					return;
				}
