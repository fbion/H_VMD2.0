...
dhtmlXGridObject.prototype.splitAt=function(ind){
    ...
    //debugger
    this._fake=new dhtmlXGridObject(leftBox); //左侧锁定列
    ...
    var b_fu = ["setHeader", "setInitWidths", "setColTypes", "setColAlign", "setColVAlign", "setColSorting", "setColumnColor", "setHeaderRowHeight"];
...
//inner methods
		...
		function proc_hf(i, rows, mode, frows) {
            //debugger
			var temp_header=(new Array(ind+1)).join(this.delim);
			var temp_rspan=[];
			if (i==2)
				for (var k=0; k<ind; k++){
...
//		 var z = this._fake.hdr.rows[1];
    //        z.height = "100";
		...
		//重新赋值表头高度 20141031
				if (this.hdr && this.initHeaderRowHeight) {
				    for (var r = 1; r < this._fake.hdr.rows.length && r <= this.initHeaderRowHeight.length; r++) {
				        var z = this._fake.hdr.rows[r];
				        if (this.initHeaderRowHeight[r - 1]==0)
                    z.style.display = 'none'; //表头隐藏
                else
                    z.height = this.initHeaderRowHeight[r - 1];
...
var quirks = (_isIE && document.compatMode=="BackCompat");
		...
		*/
            ...
            this.setSizes=function(){
                    ...
                    //2014-6-16 lhf 去掉锁定列下面的滚动条
                    if (this._fake.objBox.style.overflowX=="hidden"&&this.objBox.style.overflowX=="scroll") 
                    {
                      this._fake.objBox.style.height = (parseInt(this._fake.objBox.style.height)-17)+"px";
                    }
            ...
            this[zname]=function(){
                    this["_bfs_doClick"].apply(this,arguments);
                        if (arguments[0].tagName=="TD"){
                            ...
                            //非锁定列点击报错的屏蔽 mhb 20141014
							if (this._fake.cells(arguments[0].parentNode.idd, (fl ? 0 : arguments[0]._cellIndex))) {
